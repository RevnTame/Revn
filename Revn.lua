...
--   This Is The Source Channel @HATLLA .   --
--                 - Revn -                 --
--         -- https://t.me/HATLLA --         --
------------------------------------------------ 
...
if not DevAbs:get(Server.."IdRevn") then 
io.write('\27[1;35m\nالان ارسل ايدي المطور الاساسي ↫ ⤈\n\27[0;33;49m') 
local DevId = io.read():gsub(' ','') 
if tostring(DevId):match('%d+') then 
io.write('\27[1;36mتم حفظ ايدي المطور الاساسي\n27[0;39;49m') 
DevAbs:set(Server.."IdRevn",DevId) 
else 
print('\27[1;31m┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\nلم يتم حفظ ايدي المطور الاساسي ارسله مره اخرى\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉') 
end 
os.execute('lua Revn.lua') 
end 
if not DevAbs:get(Server.."TokenRevn") then 
...
DevAbs:set(Server.."TokenRevn",TokenBot) 
end  
else 
...
Config = {
DevId = DevAbs:get(Server.."IdRevn"),
TokenBot = DevAbs:get(Server.."TokenRevn"),
Revn = DevAbs:get(Server.."TokenRevn"):match("(%d+)"),
SudoIds = {DevAbs:get(Server.."IdRevn")},
}
Create(Config, "./config.lua") 
file = io.open("Revn.sh", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/Revn
token="]]..DevAbs:get(Server.."TokenRevn")..[["
while(true) do
rm -fr ../.telegram-cli
if [ ! -f ./tg ]; then
echo "┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉"
...
./tg -s ./Revn.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("Run", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S Revn -X kill
screen -S Revn ./Revn.sh
done
]]) 
file:close() 
...
local Load_Revn = function() 
local f = io.open("./config.lua", "r") 
if not f then 
AutoSet() 
else 
f:close() 
DevAbs:del(Server.."IdRevn");DevAbs:del(Server.."TokenRevn")
end 
...
NameBot = (DevAbs:get(Revn..'Abs:NameBot') or 'ريفن')
--     Source Revn     --
...
local Status = DevAbs:sismember(Revn..'Abs:SecondSudo:',msg.sender_user_id_) 
...
function Bot(msg) 
local var = false  
if msg.sender_user_id_ == tonumber(Revn) then  
var = true  
end  
return var  
end 
...
local Status = DevAbs:sismember(Revn..'Abs:SudoBot:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Revn..'Abs:ManagerAll:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Revn..'Abs:AdminAll:',msg.sender_user_id_) 
...
------ Vip Member All ------
function VipAll(msg) 
local Status = DevAbs:sismember(Revn..'Abs:VipAll:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Revn..'Abs:AbsConstructor:'..msg.chat_id_,msg.sender_user_id_) 
if Status or SudoBot(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
local Status = DevAbs:sismember(Revn..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Revn..'Abs:Constructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Revn..'Abs:Managers:'..msg.chat_id_,msg.sender_user_id_) 
...
----------  Admin  ---------
function Admin(msg) 
local Status = DevAbs:sismember(Revn..'Abs:Admins:'..msg.chat_id_,msg.sender_user_id_) 
...
---------Vip Member---------
function VipMem(msg) 
local Status = DevAbs:sismember(Revn..'Abs:VipMem:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Revn..'Abs:Cleaner:'..msg.chat_id_,msg.sender_user_id_) 
...
if DevAbs:sismember(Revn..'Abs:Ban:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(Revn..'Abs:BanAll:', user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(Revn..'Abs:Muted:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(Revn..'Abs:MuteAll:', user_id) then
var = true
else
var = false
end
...
--     Source Revn     --
function RevnFiles(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
plugin = dofile("Files/"..v)
if plugin.Revn and msg then
FilesText = plugin.Revn(msg)
end
end
end
...
if File_Name:lower():match('(%d+)') ~= Revn:lower() then 
...
local info_file = io.open('./'..Revn..'.json', "r"):read('*a')
local JsonInfo = JSON.decode(info_file)
vardump(JsonInfo)
DevAbs:set(Revn.."Abs:NameBot",JsonInfo.BotName) 
for IdGps,v in pairs(JsonInfo.GroupsList) do
DevAbs:sadd(Revn.."Abs:Groups",IdGps) 
DevAbs:set(Revn.."Abs:Lock:Bots"..IdGps,"del") DevAbs:hset(Revn.."Abs:Spam:Group:User"..IdGps ,"Spam:User","keed") 
...
DevAbs:set(Revn..Lock..IdGps,true)
end
if v.AbsConstructors then
for k,IdAbsConstructors in pairs(v.AbsConstructors) do
DevAbs:sadd(Revn..'Abs:AbsConstructor:'..IdGps,IdAbsConstructors)  
print('تم رفع منشئين المجموعات')
end
end
if v.BasicConstructors then
for k,IdBasicConstructors in pairs(v.BasicConstructors) do
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..IdGps,IdBasicConstructors)  
print('تم رفع ( '..k..' ) منشئين اساسيين')
end
end
if v.Constructors then
for k,IdConstructors in pairs(v.Constructors) do
DevAbs:sadd(Revn..'Abs:Constructor:'..IdGps,IdConstructors)  
print('تم رفع ( '..k..' ) منشئين')
end
end
if v.Managers then
for k,IdManagers in pairs(v.Managers) do
DevAbs:sadd(Revn..'Abs:Managers:'..IdGps,IdManagers)  
...
vardump(IdAdmins)
DevAbs:sadd(Revn..'Abs:Admins:'..IdGps,IdAdmins)  
print('تم رفع ( '..k..' ) ادمنيه')
end
end
if v.Vips then
for k,IdVips in pairs(v.Vips) do
DevAbs:sadd(Revn..'Abs:VipMem:'..IdGps,IdVips)  
print('تم رفع ( '..k..' ) مميزين')
end
end
if v.LinkGroups then
if v.LinkGroups ~= "" then
DevAbs:set(Revn.."Abs:Groups:Links"..IdGps,v.LinkGroups)   
print('( تم وضع روابط المجموعات )')
end
end
if v.Welcomes then
if v.Welcomes ~= "" then
DevAbs:set(Revn.."Abs:Groups:Welcomes"..IdGps,v.Welcomes)   
...
keyboard = {}
keyboard.inline_keyboard = {{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
...
--     Source Revn     --
local AbsRank = function(msg) if SudoId(msg.sender_user_id_) then RevnTEAM  = "المطور" elseif SecondSudo(msg) then RevnTEAM = "المطور" elseif SudoBot(msg) then RevnTEAM = "المطور" elseif ManagerAll(msg) then RevnTEAM = "المدير" elseif AdminAll(msg) then RevnTEAM = "الادمن" elseif AbsConstructor(msg) then RevnTEAM = "المالك" elseif BasicConstructor(msg) then RevnTEAM = "المنشئ" elseif Constructor(msg) then RevnTEAM = "المنشئ" elseif Manager(msg) then RevnTEAM = "المدير" elseif Admin(msg) then RevnTEAM = "الادمن" else RevnTEAM = "العضو" end return RevnTEAM end
function IdRank(user_id,chat_id) if tonumber(user_id) == tonumber(1783964439) then RevnTEAM = 'مطور السورس' elseif tonumber(user_id) == tonumber(Revn) then RevnTEAM = 'البوت' elseif SudoId(user_id) then RevnTEAM = 'المطور الاساسي' elseif DevAbs:sismember(Revn..'Abs:SecondSudo:', user_id) then RevnTEAM = 'المطور الثانوي' elseif DevAbs:sismember(Revn..'Abs:SudoBot:', user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:SudoBot:Rd"..chat_id) or 'المطور' elseif DevAbs:sismember(Revn..'Abs:ManagerAll:', user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:Managers:Rd"..chat_id) or 'المدير العام' elseif DevAbs:sismember(Revn..'Abs:AdminAll:', user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:Admins:Rd"..chat_id) or 'الادمن العام' elseif DevAbs:sismember(Revn..'Abs:VipAll:', user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:VipMem:Rd"..chat_id) or 'المميز العام' elseif DevAbs:sismember(Revn..'Abs:AbsConstructor:'..chat_id, user_id) then RevnTEAM = 'المالك' elseif DevAbs:sismember(Revn..'Abs:BasicConstructor:'..chat_id, user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:BasicConstructor:Rd"..chat_id) or 'المنشئ الاساسي' elseif DevAbs:sismember(Revn..'Abs:Constructor:'..chat_id, user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:Constructor:Rd"..chat_id) or 'المنشئ' elseif DevAbs:sismember(Revn..'Abs:Managers:'..chat_id, user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:Managers:Rd"..chat_id) or 'المدير' elseif DevAbs:sismember(Revn..'Abs:Admins:'..chat_id, user_id) then RevnTEAM = DevAbs:get(Revn.."Abs:Admins:Rd"..chat_id) or 'الادمن' elseif DevAbs:sismember(Revn..'Abs:VipMem:'..chat_id, user_id) then  RevnTEAM = DevAbs:get(Revn.."Abs:VipMem:Rd"..chat_id) or 'المميز' elseif DevAbs:sismember(Revn..'Abs:Cleaner:'..chat_id, user_id) then  RevnTEAM = DevAbs:get(Revn.."Abs:Cleaner:Rd"..chat_id) or 'المنظف' else RevnTEAM = DevAbs:get(Revn.."Abs:mem:Rd"..chat_id) or 'العضو' end return RevnTEAM end
--     Source Revn     --
function RankChecking(user_id,chat_id)
if SudoId(user_id) then
var = true  
elseif tonumber(user_id) == tonumber(Revn) then  
var = true  
elseif DevAbs:sismember(Revn..'Abs:SecondSudo:', user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:SudoBot:', user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:ManagerAll:', user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:AdminAll:', user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:VipAll:', user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:AbsConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(Revn..'Abs:BasicConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(Revn..'Abs:Constructor:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:Managers:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:Admins:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(Revn..'Abs:VipMem:'..chat_id, user_id) then  
var = true 
else  
var = false
end  
...
local abs = (Revn..'Abs:Filters:'..msg.chat_id_)
if abs then
...
local UserName = (dp.username_ or "HATLLA")
...
local var = true 
if DevAbs:get(Revn.."Abs:ChId") then
local url , res = https.request('https://api.telegram.org/bot'..TokenBot..'/getchatmember?chat_id='..DevAbs:get(Revn..'Abs:ChId')..'&user_id='..msg.sender_user_id_)
local data = json:decode(url)
if res ~= 200 or data.result.status == "left" or data.result.status == "kicked" then
var = false 
if DevAbs:get(Revn..'Abs:ChText') then
local ChText = DevAbs:get(Revn..'Abs:ChText')
send(msg.chat_id_,msg.id_,'['..ChText..']')
else
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevAbs:get(Revn.."Abs:ChId"))
local GetInfo = JSON.decode(Check)
...
if DataText == '/delyes' and DevAbs:get(Revn..'yes'..data.sender_user_id_) == 'delyes' then
DevAbs:del(Revn..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(Revn..'no'..data.sender_user_id_, 'delno')
...
if DataText == '/delno' and DevAbs:get(Revn..'no'..data.sender_user_id_) == 'delno' then
DevAbs:del(Revn..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(Revn..'no'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "⌔︙تم الغاء امر اطردني") 
end
--     Source Revn     --
if DataText == '/yesdel' and DevAbs:get(Revn..'yesdel'..data.sender_user_id_) == 'delyes' then
DevAbs:del(Revn..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(Revn..'nodel'..data.sender_user_id_, 'delno')
if DevAbs:sismember(Revn..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(Revn..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) then
Managers = 'المدراء • ' else Managers = '' end
if DevAbs:sismember(Revn..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(Revn..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(Revn..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) then
cleaner = 'المنظفين • ' else cleaner = '' end
if DevAbs:sismember(Revn..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
donky = 'المطايه • ' else donky = '' end
if DevAbs:sismember(Revn..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Revn..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Revn..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Revn..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Revn..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Revn..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
DevAbs:srem(Revn..'Abs:Constructor:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Revn..'Abs:Managers:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Revn..'Abs:Admins:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Revn..'Abs:VipMem:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Revn..'Abs:Cleaner:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Revn..'User:Donky:'..data.chat_id_,data.sender_user_id_)
...
if DevAbs:get(Revn.."Abs:NewDev"..data.sender_user_id_) then
if DataText == '/setno' then
EditMsg(Chat_Id2, Msg_Id2, "⌔︙تم الغاء امر تغير المطور الاساسي") 
DevAbs:del(Revn.."Abs:NewDev"..data.sender_user_id_)
return false
end
if DataText == '/setyes' then
local NewDev = DevAbs:get(Revn.."Abs:NewDev"..data.sender_user_id_)
...
if DataText == '/nodel' and DevAbs:get(Revn..'nodel'..data.sender_user_id_) == 'delno' then
DevAbs:del(Revn..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(Revn..'nodel'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "⌔︙تم الغاء امر نزلني") 
end
if DataText == '/YesRolet' and DevAbs:get(Revn.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_) then
local List = DevAbs:smembers(Revn..'Abs:ListRolet'..data.chat_id_) 
local UserName = List[math.random(#List)]
tdcli_function ({ID="SearchPublicChat",username_ = UserName},function(arg,dp) 
DevAbs:incrby(Revn..'Abs:GamesNumber'..data.chat_id_..dp.id_, 5) 
end,nil) 
DevAbs:del(Revn..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(Revn.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
EditMsg(Chat_Id2, Msg_Id2, "⌔︙*صاحب الحظ* ↫ ["..UserName.."]\n⌔︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*")
end
if DataText == '/NoRolet' then
DevAbs:del(Revn..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(Revn.."Abs:NumRolet"..data.chat_id_..data.sender_user_id_) 
DevAbs:del(Revn.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
...
if DevAbs:sismember(Revn..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..Chat_Id2.."&user_id="..data.sender_user_id_.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
DevAbs:srem(Revn..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_)
...
DevAbs:sadd(Revn..'Abs:VipMem:'..data.chat_id_,dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'⌔︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'HATLLA')..')\n⌔︙تم رفعه في قائمة المميزين')
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:AbsConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(Revn..'Abs:SecondSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:AbsConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:SecondSudo:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(Revn..'Abs:Muted:'..data.chat_id_,dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'⌔︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'HATLLA')..')\n⌔︙تم كتمه من المجموعه')
end
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:Muted:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(Revn..'Abs:Ban:'..data.chat_id_,dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'⌔︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'HATLLA')..')\n⌔︙تم حظره من المجموعه')
end
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(Revn..'Abs:Ban:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(Revn..'Abs:Tkeed:'..data.chat_id_,dp.id_)
...
DevAbs:srem(Revn..'Abs:Tkeed:'..data.chat_id_,dp.id_)
...
keyboard = {} 
if Sudo(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text="تنزيل منشئ",callback_data=data.sender_user_id_..":RemConstructor:"..AbsId},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..AbsId}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..AbsId},{text="تنزيل منشئ اساسي",callback_data=data.sender_user_id_..":RemBasicConstructor:"..AbsId}},{{text="تنزيل مالك",callback_data=data.sender_user_id_..":RemAbsConstructor:"..AbsId},{text="تنزيل مطور",callback_data=data.sender_user_id_..":RemSudoBot:"..AbsId}},{{text="تنزيل مطور ثانوي",callback_data=data.sender_user_id_..":RemSecondSudo:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif SecondSudo(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text="تنزيل منشئ",callback_data=data.sender_user_id_..":RemConstructor:"..AbsId},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..AbsId}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..AbsId},{text="تنزيل منشئ اساسي",callback_data=data.sender_user_id_..":RemBasicConstructor:"..AbsId}},{{text="تنزيل مالك",callback_data=data.sender_user_id_..":RemAbsConstructor:"..AbsId},{text="تنزيل مطور",callback_data=data.sender_user_id_..":RemSudoBot:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif SudoBot(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text="تنزيل منشئ",callback_data=data.sender_user_id_..":RemConstructor:"..AbsId},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..AbsId}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..AbsId},{text="تنزيل منشئ اساسي",callback_data=data.sender_user_id_..":RemBasicConstructor:"..AbsId}},{{text="تنزيل مالك",callback_data=data.sender_user_id_..":RemAbsConstructor:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif AbsConstructor(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text="تنزيل منشئ",callback_data=data.sender_user_id_..":RemConstructor:"..AbsId},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..AbsId}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..AbsId},{text="تنزيل منشئ اساسي",callback_data=data.sender_user_id_..":RemBasicConstructor:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif BasicConstructor(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text="تنزيل منشئ",callback_data=data.sender_user_id_..":RemConstructor:"..AbsId},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..AbsId}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Constructor(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..AbsId},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Manager(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Admin(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..AbsId}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
end 
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end,nil)
end
...
DevAbs:del(Revn..'Abs:Text:GpTexts'..Abbs..data.chat_id_)
DevAbs:srem(Revn..'Abs:Manager:GpRedod'..data.chat_id_,Abbs)
end
if DataText and DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)') then
local Abbs = DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)')
local List = DevAbs:smembers(Revn..'Abs:Text:GpTexts'..Abbs..data.chat_id_)
if DevAbs:get(Revn..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "⌔︙تم انهاء وحفظ ↫ "..#List.." من الردود المتعدده للامر ↫ "..Abbs) 
DevAbs:del(Revn..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
DevAbs:del(Revn..'Abs:Text:GpTexts'..Abbs..data.chat_id_)
DevAbs:del(Revn..'Abs:Add:GpTexts'..data.sender_user_id_..data.chat_id_)
DevAbs:srem(Revn..'Abs:Manager:GpRedod'..data.chat_id_,Abbs)
else
EditMsg(Chat_Id2, Msg_Id2, "⌔︙عذرا صلاحية الامر منتهيه !") 
end
...
if tonumber(Abbs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(Revn..'Abs:Help')
local Text = [[
...
⌔︙م6 ↫ اوامر الاعضاء
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
⌔︙[Source Channel](https://t.me/HATLLA)
]] 
keyboard = {} 
...
local Help = DevAbs:get(Revn..'Abs:Help1')
local Text = [[
...
local Help = DevAbs:get(Revn..'Abs:Help2')
local Text = [[
⌔︙اوامر الادمنيه ↫ ⤈
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
⌔︙الاعدادت
...
local Help = DevAbs:get(Revn..'Abs:Help3')
local Text = [[
⌔︙اوامر المدراء ↫ ⤈
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
⌔︙فحص البوت
...
local Help = DevAbs:get(Revn..'Abs:Help4')
local Text = [[
⌔︙اوامر المنشئين ↫ ⤈
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
⌔︙تنزيل الكل
...
local Help = DevAbs:get(Revn..'Abs:Help5')
local Text = [[
⌔︙اوامر المطورين ↫ ⤈
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
⌔︙الكروبات
...
if tonumber(Abbs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(Revn..'Abs:Help6')
local Text = [[
⌔︙اوامر الاعضاء ↫ ⤈
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
...
if text and DevAbs:get(Revn.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = DevAbs:get(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
DevAbs:del(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
DevAbs:del(Revn.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:srem(Revn.."List:Cmd:Group:New"..msg.chat_id_,text)
...
DevAbs:del(Revn.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if text and text:match('^'..(DevAbs:get(Revn..'Abs:NameBot') or "ريفن")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(DevAbs:get(Revn..'Abs:NameBot') or "ريفن")..' ','')
end
if data.message_.content_.text_ then
local NewCmmd = DevAbs:get(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":"..data.message_.content_.text_)
if NewCmmd then
data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
end
end
if text and DevAbs:get(Revn.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
DevAbs:set(Revn.."Set:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙ارسل الامر الجديد", 1, 'html')
DevAbs:del(Revn.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
DevAbs:set(Revn.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_,"true1") 
return false
end
if text and DevAbs:get(Revn.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
local NewCmd = DevAbs:get(Revn.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text,NewCmd)
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الامر", 1, 'html')
DevAbs:del(Revn.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if Constructor(msg) then
if text == "الاوامر المضافه" and ChCheck(msg) then
local List = DevAbs:smembers(Revn.."List:Cmd:Group:New"..msg.chat_id_.."") 
t = "⌔︙قائمة الاوامر المضافه ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
Cmds = DevAbs:get(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
if Cmds then 
t = t..k.."~ ("..v..") • {"..Cmds.."}\n"
else
t = t..k.."~ ("..v..") \n"
...
local List = DevAbs:smembers(Revn.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
DevAbs:del(Revn.."List:Cmd:Group:New"..msg.chat_id_)
end
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حذف الاوامر المضافه في المجموعه", 1, 'html')
end
if text == "ترتيب الاوامر" then
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":ا","ايدي")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"ا")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":تك","تنزيل الكل")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"تك")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":م","رفع مميز")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"م")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":اد","رفع ادمن")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"اد")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":مد","رفع مدير")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"مد")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":من","رفع منشئ")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"من")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":اس","رفع منشئ اساسي")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"اس")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":تعط","تعطيل الايدي بالصوره")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"تعط")
DevAbs:set(Revn.."Set:Cmd:Group:New1"..msg.chat_id_..":تفع","تفعيل الايدي بالصوره")
DevAbs:sadd(Revn.."List:Cmd:Group:New"..msg.chat_id_,"تفع")
send(msg.chat_id_, msg.id_,"⌔︙تم ترتيب الاوامر بالشكل التالي ↫ ⤈\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n⌔︙ايدي ↫ ا\n⌔︙تنزيل الكل ↫ تك\n⌔︙رفع مميز ↫ م\n⌔︙رفع ادمن ↫ اد \n⌔︙رفع مدير ↫ مد \n⌔︙رفع منشئ ↫ من \n⌔︙رفع منشئ اساسي ↫ اس  \n⌔︙تفعيل الايدي بالصوره ↫ تفع\n⌔︙تعطيل الايدي بالصوره ↫ تعط\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉")  
end
if text == "اضف امر" or text == "اضافة امر" or text == "اضافه امر" and ChCheck(msg) then
DevAbs:set(Revn.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙ارسل الامر القديم", 1, 'html')
return false
end
if text == "حذف امر" or text == "مسح امر" and ChCheck(msg) then 
DevAbs:set(Revn.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
local List = DevAbs:smembers(Revn.."Coomds"..msg.chat_id_)
if #List == 0 then
...
for k,v in pairs(List) do
var = DevAbs:get(Revn.."Comd:New:rt:Abs:"..v..msg.chat_id_)
if var then
t = t..k.."~ "..v.." • ("..var..")\n"
else
t = t..k.."~ "..v.."\n"
...
local List = DevAbs:smembers(Revn.."Coomds"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(Revn.."Comd:New:rt:Abs:"..v..msg.chat_id_)
DevAbs:del(Revn.."Coomds"..msg.chat_id_)
end
...
DevAbs:set(Revn.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_,ComdNew)  
DevAbs:sadd(Revn.."Coomds"..msg.chat_id_,ComdNew)  
DevAbs:setex(Revn.."Comd:New"..msg.chat_id_..msg.sender_user_id_,200,true)  
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙ارسل نوع الصلاحيه \n{ عضو • مميز  • ادمن  • مدير }\n⌔︙ارسل الغاء لالغاء الامر ", 1, 'html')
end
if text and text:match("^حذف صلاحيه (.*)$") and ChCheck(msg) or text and text:match("^مسح صلاحيه (.*)$") and ChCheck(msg) then 
ComdNew = text:match("^حذف صلاحيه (.*)$") or text:match("^مسح صلاحيه (.*)$")
DevAbs:del(Revn.."Comd:New:rt:Abs:"..ComdNew..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حذف الصلاحيه", 1, 'html')
end
if DevAbs:get(Revn.."Comd:New"..msg.chat_id_..msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء الامر", 1, 'html')
DevAbs:del(Revn.."Comd:New"..msg.chat_id_..msg.sender_user_id_) 
return false  
end 
if text == "مدير" then
...
local textn = DevAbs:get(Revn.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_)  
DevAbs:set(Revn.."Comd:New:rt:Abs:"..textn..msg.chat_id_,text)
...
if DevAbs:sismember(Revn.."Coomds"..msg.chat_id_,DEV_ABBAS) then
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local mrabs = DevAbs:get(Revn.."Comd:New:rt:Abs:"..DEV_ABBAS..msg.chat_id_)
if mrabs == "مميز" and VipMem(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:set(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_ABBAS) 
DevAbs:sadd(Revn..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:set(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_ABBAS)
DevAbs:sadd(Revn..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:set(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_ABBAS)  
DevAbs:sadd(Revn..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(Revn..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'HATLLA')..')'..' ❩\n⌔︙تم تنزيله ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:srem(Revn..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'HATLLA')..')'..' ❩\n⌔︙تم تنزيله ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:srem(Revn..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrabs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'HATLLA')..')'..' ❩\n⌔︙تم تنزيله ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
end
...
if DevAbs:sismember(Revn.."Coomds"..msg.chat_id_,text1[2]) then
function py_username(extra, result, success)   
if result.id_ then
local mrabs = DevAbs:get(Revn.."Comd:New:rt:Abs:"..text1[2]..msg.chat_id_)
if mrabs == "مميز" and VipMem(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(Revn..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:set(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(Revn..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:set(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(Revn..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:set(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrabs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevAbs:srem(Revn..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:del(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:srem(Revn..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:del(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:srem(Revn..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:del(Revn.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrabs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'HATLLA')..')'..' ❩\n⌔︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevAbs:incr(Revn..'Abs:UsersMsgs'..Revn..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(Revn..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d'))  
ChatType = 'sp' 
elseif id:match("^(%d+)") then
if not DevAbs:sismember(Revn.."Abs:Users",msg.chat_id_) then
DevAbs:sadd(Revn.."Abs:Users",msg.chat_id_)
end
ChatType = 'pv' 
else
ChatType = 'gp' 
...
if SecondSudo(msg) then 
local Sudo_Welcome = '⌔︙اهلا بك مجددا عزيزي المطور \n⌔︙اليك الازرار الخاصه بتعديل وتغيير كلايش سورس ريفن فقط اضغط على الامر الذي تريد تنفيذه'
local key = {
...
if not DevAbs:get(Revn..'Abs:Start:Time'..msg.sender_user_id_) then
tdcli_function({ID="GetUser",user_id_=Revn},function(arg,dp) 
local inline = {{{text="⌁ اضفني في مجموعتك ⌁",url="t.me/"..dp.username_.."?startgroup=botstart"}}}
local start = DevAbs:get(Revn.."Abs:Start:Bot")
if start then 
Start_Source = start
else
...
DevAbs:setex(Revn..'Abs:Start:Time'..msg.sender_user_id_,300,true)
return false
end 
--     Source Revn     --
if not SecondSudo(msg) and not DevAbs:sismember(Revn..'Abs:Ban:Pv',msg.sender_user_id_) and not DevAbs:get(Revn..'Abs:Texting:Pv') then
tdcli_function({ID="GetUser",user_id_=DevId},function(arg,chat) 
Dev_Abs(msg.sender_user_id_, msg.id_, 1, '⌔︙تم ارسال رسالتك الى [المطور](t.me/'..(chat.username_ or "HATLLA")..')', 1, 'md') 
...
DevAbs:sadd(Revn..'Abs:Ban:Pv',data.id_)  
return false  
end 
if text == 'الغاء الحظر' or text == 'الغاء حظر' then
local Text = '⌔︙العضو ↫ ['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..data.id_..')'..'\n⌔︙تم الغاء حظره من التواصل'
SendText(DevId,Text,msg.id_/2097152/0.5,'md') 
DevAbs:srem(Revn..'Abs:Ban:Pv',data.id_)  
return false  
end 
...
--     Source Revn     --
if text and DevAbs:get(Revn..'Abs:Start:Bots'..msg.sender_user_id_) then
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ كليشة الستارت', 1, 'md')
DevAbs:del(Revn..'Abs:Start:Bots'..msg.sender_user_id_) 
return false
end
DevAbs:set(Revn.."Abs:Start:Bot",text)  
...
DevAbs:set(Revn..'Abs:Start:Bots'..msg.sender_user_id_,true) 
...
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل التواصل بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Texting:Pv') 
end
if text == 'تعطيل التواصل' or text == '↫ تعطيل التواصل ⌁' then  
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل التواصل بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Texting:Pv',true) 
end
end
--     Source Revn     --
...
DevAbs:incr(Revn..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(Revn.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = DevAbs:get(Revn.."Abs:Lock:Bots"..msg.chat_id_) 
for i=0,#mem_id do  
...
DevAbs:sadd(Revn..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_)
GetInfo = https.request("https://api.telegram.org/bot"..TokenBot.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local JsonInfo = JSON.decode(GetInfo)
if JsonInfo.ok == true and #mem_id == i then
local Msgs = {}
...
if msg.content_.ID == "MessageChatDeleteMember" and tonumber(msg.content_.user_.id_) == tonumber(Revn) then 
DevAbs:srem(Revn.."Abs:Groups", msg.chat_id_) 
...
if DevAbs:get(Revn..'Abs:Lock:TagServr'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})    
end   
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then   
DevAbs:incr(Revn..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d'))  
elseif msg.content_.ID == "MessageChatDeleteMember" then   
DevAbs:incr(Revn..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d'))  
end
--     Source Revn     --
if text ==('تفعيل') and not SudoBot(msg) and ChCheck(msg) then
if not DevAbs:get(Revn..'Abs:Lock:FreeBot'..Revn) then
if ChatType == 'pv' then
...
DevAbs:sadd(Revn..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(Revn..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
DevAbs:sadd(Revn..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
...
DevAbs:sadd(Revn.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:sadd(Revn.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(Revn.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:srem(Revn.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevAbs:sismember(Revn..'Abs:Groups',msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙المجموعه بالتاكيد مفعله', 1, 'md')
else
if tonumber(data.member_count_) < tonumber(DevAbs:get(Revn..'Abs:Num:Add:Bot') or 0) and not SecondSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙عدد اعضاء المجموعه اقل من ↫ *'..(DevAbs:get(Revn..'Abs:Num:Add:Bot') or 0)..'* عضو', 1, 'md')
return false
end
ReplyStatus(msg,result.id_,"ReplyBy","⌔︙تم تفعيل المجموعه "..dp.title_)  
DevAbs:sadd(Revn.."Abs:Groups",msg.chat_id_)
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
DevAbs:set(Revn.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
DevAbs:set(Revn..'Save:UserName'..msg.sender_user_id_,data.username_)
end;end,nil) 
--     Source Revn     --
local ReFalse = tostring(msg.chat_id_)
if not DevAbs:sismember(Revn.."Abs:Groups",msg.chat_id_) and not ReFalse:match("^(%d+)") and not SudoBot(msg) then
...
if DevAbs:get(Revn..'Abs:Lock:Robot'..msg.chat_id_) then
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,dp) 
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..dp.id_)
DevAbs:sadd(Revn..'Abs:Tkeed:'..msg.chat_id_, dp.id_)
...
if DevAbs:get(Revn.."Abs:Lock:Join"..msg.chat_id_) then
...
if not Manager(msg) then 
local filter = DevAbs:smembers(Revn.."Abs:FilterPhoto"..msg.chat_id_)
...
if not Manager(msg) then 
local filter = DevAbs:smembers(Revn.."Abs:FilterAnimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if not Manager(msg) then 
local filter = DevAbs:smembers(Revn.."Abs:FilterSteckr"..msg.chat_id_)
for k,v in pairs(filter) do
...
local DelGpRedRedods = DevAbs:get(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
local GetGpTexts = DevAbs:get(Revn..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
if DelGpRedRedods == 'DelGpRedRedods' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙الرد ↫ '..msg.content_.text_..' للكلمه ↫ '..GetGpTexts..' تم حذفها',  1, "html")
DevAbs:del(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(Revn..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRedod' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙الكلمه ↫ '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Text:GpTexts'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(Revn..'Abs:Manager:GpRedod'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙الكلمه ↫ '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(Revn..'Abs:Manager:GpRed'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelAllRed = DevAbs:get(Revn.."Abs:Add:AllRed"..msg.sender_user_id_)
if DelAllRed == 'DelAllRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙الكلمه ↫ '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(Revn.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(Revn.."Abs:Gif:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Voice:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Audio:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Photo:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Stecker:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Video:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:File:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Text:AllRed"..msg.content_.text_)
DevAbs:del(Revn.."Abs:Sudo:AllRed",msg.content_.text_)
return false
end
end
--     Source Revn     --
if text and text:match("^(.*)$") then
local SaveGpRedod = DevAbs:get(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRedod == 'SaveGpRedod' then
local GetGpTexts = DevAbs:get(Revn..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevAbs:smembers(Revn..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙⌔︙تم الغاء عملية حفظ الردود المتعدده للامر ↫ "..GetGpTexts ,  1, "md")
DevAbs:del(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
DevAbs:del(Revn..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(Revn..'Abs:Manager:GpRedod'..msg.chat_id_,GetGpTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevAbs:sadd(Revn..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,Text)
if #List == 4 then 
...
if text and not DevAbs:get(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_) then
if DevAbs:sismember(Revn..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
local RevnTEAM =  DevAbs:smembers(Revn..'Abs:Text:GpTexts'..text..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '['..RevnTEAM[math.random(#RevnTEAM)]..']' , 1, 'md')  
end
end
--     Source Revn     --
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then 
local SaveGpRed = DevAbs:get(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRed == 'SaveGpRed' then 
if text == 'الغاء' then
local DelManagerRep = DevAbs:get(Revn..'DelManagerRep'..msg.chat_id_)
DevAbs:srem(Revn..'Abs:Manager:GpRed'..msg.chat_id_,DelManagerRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(Revn..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Revn..'DelManagerRep'..msg.chat_id_)
return false
end
DevAbs:del(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
local SaveGpRed = DevAbs:get(Revn..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
if msg.content_.video_ then DevAbs:set(Revn..'Abs:Video:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then DevAbs:set(Revn..'Abs:File:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then DevAbs:set(Revn..'Abs:Stecker:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_) 
end 
if msg.content_.voice_ then DevAbs:set(Revn..'Abs:Voice:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_) 
end
if msg.content_.audio_ then DevAbs:set(Revn..'Abs:Audio:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_) 
end
...
DevAbs:set(Revn..'Abs:Photo:GpRed'..SaveGpRed..msg.chat_id_, photo_in_group) 
end
if msg.content_.animation_ then DevAbs:set(Revn..'Abs:Gif:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_) 
end 
if msg.content_.text_ then
DevAbs:set(Revn..'Abs:Text:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.text_)
end 
...
if msg.content_.text_ and not DevAbs:get(Revn..'Abs:Lock:GpRed'..msg.chat_id_) then 
if DevAbs:get(Revn..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Revn..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(Revn..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Revn..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(Revn..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(Revn..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(Revn..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(Revn..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(Revn..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Revn..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_))
end
if DevAbs:get(Revn..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_) then
function RevnTEAM(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(Revn..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(Revn..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
...
getUser(msg.sender_user_id_, RevnTEAM)
end
end
--     Source Revn     --
text = msg.content_.text_
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then
local SaveAllRed = DevAbs:get(Revn.."Abs:Add:AllRed"..msg.sender_user_id_)
if SaveAllRed == 'SaveAllRed' then
if text == 'الغاء' then
local DelSudoRep = DevAbs:get(Revn..'DelSudoRep')
DevAbs:del(Revn.."Abs:Sudo:AllRed",DelSudoRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(Revn.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(Revn.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(Revn.."DelSudoRep")
return false
end
DevAbs:del(Revn.."Abs:Add:AllRed"..msg.sender_user_id_)
local SaveAllRed = DevAbs:get(Revn.."Abs:Add:AllText"..msg.sender_user_id_)
if msg.content_.video_ then
DevAbs:set(Revn.."Abs:Video:AllRed"..SaveAllRed, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then
DevAbs:set(Revn.."Abs:File:AllRed"..SaveAllRed, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then
DevAbs:set(Revn.."Abs:Stecker:AllRed"..SaveAllRed, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
DevAbs:set(Revn.."Abs:Voice:AllRed"..SaveAllRed, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.audio_ then
DevAbs:set(Revn.."Abs:Audio:AllRed"..SaveAllRed, msg.content_.audio_.audio_.persistent_id_)
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
DevAbs:set(Revn.."Abs:Photo:AllRed"..SaveAllRed, photo_in_all_groups)
end
if msg.content_.animation_ then
DevAbs:set(Revn.."Abs:Gif:AllRed"..SaveAllRed, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
DevAbs:set(Revn.."Abs:Text:AllRed"..SaveAllRed, msg.content_.text_)
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حفظ الرد الجديد', 1, 'md') 
DevAbs:del(Revn.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(Revn..'DelSudoRep')
return false end end
if msg.content_.text_ and not DevAbs:get(Revn..'Abs:Lock:AllRed'..msg.chat_id_) then
if DevAbs:get(Revn.."Abs:Video:AllRed"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Revn.."Abs:Video:AllRed"..msg.content_.text_))
end
if DevAbs:get(Revn.."Abs:File:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Revn.."Abs:File:AllRed"..msg.content_.text_))
end
if DevAbs:get(Revn.."Abs:Voice:AllRed"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn.."Abs:Voice:AllRed"..msg.content_.text_))
end
if DevAbs:get(Revn.."Abs:Audio:AllRed"..msg.content_.text_)  then
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn.."Abs:Audio:AllRed"..msg.content_.text_))
end
if DevAbs:get(Revn.."Abs:Photo:AllRed"..msg.content_.text_)  then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn.."Abs:Photo:AllRed"..msg.content_.text_))
end
if  DevAbs:get(Revn.."Abs:Gif:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Revn.."Abs:Gif:AllRed"..msg.content_.text_))
end
if DevAbs:get(Revn.."Abs:Stecker:AllRed"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Revn.."Abs:Stecker:AllRed"..msg.content_.text_))
end
if DevAbs:get(Revn.."Abs:Text:AllRed"..msg.content_.text_) then
function RevnTEAM(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(Revn..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(Revn.."Abs:Text:AllRed"..msg.content_.text_)
local Text = Text:gsub('#username',(username or 'لا يوجد')) 
local Text = Text:gsub('#name','['..result.first_name_..']')
local Text = Text:gsub('#id',msg.sender_user_id_)
local Text = Text:gsub('#edit',edit_msg)
...
if Type == "keed" and not DevAbs:sismember(Revn..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_) then
...
if Type == "mute" and not DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, msg.sender_user_id_) then
DevAbs:sadd(Revn..'Abs:Muted:'..msg.chat_id_,msg.sender_user_id_)
my_ide = msg.sender_user_id_
msgm = msg.id_
local num = 100
for i=1,tonumber(num) do
...
--     Source Revn     --
--       Spam Check       --
if not VipMem(msg) and msg.content_.ID ~= "MessageChatAddMembers" and DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= Revn then
floods = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(DevAbs:get(Revn.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
DevAbs:setex(Revn.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
--     Source Revn     --
...
if Constructor(msg) or tonumber(msg.sender_user_id_) == tonumber(Revn) then
DevAbs:set(Revn..'Abs:PinnedMsg'..msg.chat_id_,msg.content_.message_id_)
else
local pin_id = DevAbs:get(Revn..'Abs:PinnedMsg'..msg.chat_id_)
if pin_id and DevAbs:get(Revn..'Abs:Lock:Pin'..msg.chat_id_) then
pinmsg(msg.chat_id_,pin_id,0)
end
end
end
if DevAbs:get(Revn..'Abs:viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
DevAbs:del(Revn..'Abs:viewget'..msg.sender_user_id_)
else
...
if DevAbs:get(Revn..'Abs:Lock:Forwards'..msg.chat_id_) then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
if DevAbs:get(Revn..'Abs:Lock:Photo'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
if DevAbs:get(Revn..'Abs:Lock:Links'..msg.chat_id_) then
if msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if DevAbs:get(Revn..'Abs:Lock:Tags'..msg.chat_id_) then
if msg.content_.caption_:match("@") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("#") then
if DevAbs:get(Revn..'Abs:Lock:Hashtak'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://") or msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Xx][Yy][Zz]") or msg.content_.caption_:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
if DevAbs:get(Revn..'Abs:Lock:WebLinks'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[\216-\219][\128-\191]") then
if DevAbs:get(Revn..'Abs:Lock:Arabic'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]") then
if DevAbs:get(Revn..'Abs:Lock:English'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if DevAbs:get(Revn..'Abs:Lock:Markdown'..msg.chat_id_) then
if not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(Revn..'Abs:Lock:Document'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if not VipMem(msg) then
if DevAbs:get(Revn..'Abs:Lock:Inline'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source Revn     --
...
elseif msg.content_.ID == "MessageSticker" then
if not VipMem(msg) then
if DevAbs:get(Revn..'Abs:Lock:Stickers'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(Revn..'Abs:Groups:Welcomes'..msg.chat_id_) then
Welcomes = DevAbs:get(Revn..'Abs:Groups:Welcomes'..msg.chat_id_)
else
Welcomes = '• نورت حبي \n• firstname \n• username'
end
local Welcomes = Welcomes:gsub('"',"") Welcomes = Welcomes:gsub("'","") Welcomes = Welcomes:gsub(",","") Welcomes = Welcomes:gsub("*","") Welcomes = Welcomes:gsub(";","") Welcomes = Welcomes:gsub("`","") Welcomes = Welcomes:gsub("{","") Welcomes = Welcomes:gsub("}","") 
local Welcomes = Welcomes:gsub('firstname',('['..result.first_name_..']' or ''))
local Welcomes = Welcomes:gsub('username',('[@'..result.username_..']' or '[@HATLLA]'))
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
end 
if DevAbs:get(Revn.."Abs:Lock:Welcome"..msg.chat_id_) then
getUser(msg.sender_user_id_,get_welcome)
end
--     Source Revn     --
--      New User Add      --
elseif msg.content_.ID == "MessageChatAddMembers" then
if not DevAbs:get(Revn..'Abs:Lock:BotWelcome') then 
tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = Revn,offset_ = 0,limit_ = 1},function(extra,abbas,success) 
for i=0,#msg.content_.members_ do    
BotWelcome = msg.content_.members_[i].id_    
if BotWelcome and BotWelcome == tonumber(Revn) then 
if DevAbs:sismember(Revn..'Abs:Groups',msg.chat_id_) then BotText = "مفعله في السابق\n⌔︙ارسل ↫ الاوامر واستمتع بالمميزيات" else BotText = "معطله يجب رفعي مشرف\n⌔︙بعد ذلك يرجى ارسال امر ↫ تفعيل\n⌔︙سيتم رفع الادمنيه والمنشئ تلقائيا" end 
if DevAbs:get(Revn.."Abs:Text:BotWelcome") then AbsText = DevAbs:get(Revn.."Abs:Text:BotWelcome") else AbsText = "⌔︙مرحبا انا بوت اسمي "..NameBot.."\n⌔︙حالة المجموعه ↫ "..BotText.."\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉" end 
if DevAbs:get(Revn.."Abs:Photo:BotWelcome") then AbsPhoto = DevAbs:get(Revn.."Abs:Photo:BotWelcome") elseif abbas.photos_[0] then AbsPhoto = abbas.photos_[0].sizes_[1].photo_.persistent_id_ else AbsPhoto = nil end 
...
local Welcomes = Welcomes:gsub('username',('[@'..msg.content_.members_[0].username_..']' or '[@HATLLA]'))
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
end
--     Source Revn     --
--        Contact         --
...
if DevAbs:get(Revn..'Abs:Lock:Contact'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source Revn     --
...
if DevAbs:get(Revn..'Abs:Lock:Music'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Revn..'Abs:Lock:Voice'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Revn..'Abs:Lock:Location'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
return
end
if msg.content_.caption_ then
...
if DevAbs:get(Revn..'Abs:Lock:Videos'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Revn..'Abs:Lock:Gifs'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Revn..'Abs:Lock:Text'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
...
if not DevAbs:get(Revn..'Abs:Spam:Text'..msg.chat_id_) then
sens = 400
else
sens = tonumber(DevAbs:get(Revn..'Abs:Spam:Text'..msg.chat_id_))
end
if DevAbs:get(Revn..'Abs:Lock:Spam'..msg.chat_id_) and string.len(msg.content_.text_) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
--     Source Revn     --
if DevAbs:get(Revn.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_,"⌔︙تم الغاء حفظ الرابط")       
DevAbs:del(Revn.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) 
return false
end
if msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)") then
local Link = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
DevAbs:set(Revn.."Abs:Groups:Links"..msg.chat_id_,Link)
...
if DevAbs:get('RevnTEAM:'..Revn.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('RevnTEAM:'..Revn..'id:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء الامر', 1, 'md')
DevAbs:del('RevnTEAM:'..Revn.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('RevnTEAM:'..Revn.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('RevnTEAM:'..Revn..'id:user'..msg.chat_id_)  
DevAbs:incrby(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..iduserr,numadded)
Dev_Abs(msg.chat_id_, msg.id_,  1, "⌔︙تم اضافة "..numadded..' رساله', 1, 'md')
DevAbs:del('RevnTEAM:'..Revn..'id:user'..msg.chat_id_) 
end
end
if text and Constructor(msg) then 
if DevAbs:get('RevnTEAM:'..Revn.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('RevnTEAM:'..Revn..'ids:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء الامر', 1, 'md')
DevAbs:del('RevnTEAM:'..Revn.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('RevnTEAM:'..Revn.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('RevnTEAM:'..Revn..'ids:user'..msg.chat_id_)  
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..iduserr,numadded)  
...
if not DevAbs:get(Revn.."Abs:Lock:Fshar"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if DevAbs:get(Revn.."Abs:Lock:Farsi"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ReplyStatus(msg,msg.sender_user_id_,"WrongWay","⌔︙ممنوع التكلم بالغه الفارسيه هنا")  
end end
if text and (text:match("ڬ") or text:match("ٺ") or text:match("چ") or text:match("ڇ") or text:match("ڿ") or text:match("ڀ") or text:match("ڎ") or text:match("ݫ") or text:match("ژ") or text:match("ڟ") or text:match("ݜ") or text:match("ڸ") or text:match("پ") or text:match("۴") or text:match("مک") or text:match("زدن") or text:match("دخترا") or text:match("دیوث") or text:match("کلیپشن") or text:match("خوششون") or text:match("میدا") or text:match("که") or text:match("بدانیم") or text:match("باید") or text:match("زناشویی") or text:match("آموزش") or text:match("راحتی") or text:match("خسته") or text:match("بیام") or text:match("بپوشم") or text:match("كرمه")) then
if DevAbs:get(Revn.."Abs:Lock:FarsiBan"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ChatKick(msg.chat_id_, msg.sender_user_id_)
end end 
if text and (text:match("خره بالله") or text:match("خبربك") or text:match("كسدينربك") or text:match("خرب بالله") or text:match("خرب الله") or text:match("خره بربك") or text:match("الله الكواد") or text:match("خره بمحمد") or text:match("كسم الله") or text:match("كسم ربك") or text:match("كسربك") or text:match("كسختالله") or text:match("كسخت الله") or text:match("خره بدينك") or text:match("خرهبدينك") or text:match("كسالله") or text:match("خربالله")) then
if not DevAbs:get(Revn.."Abs:Lock:Kfr"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if not DevAbs:get(Revn.."Abs:Lock:Taf"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
local List = DevAbs:smembers(Revn..'Abs:Groups') 
local BotName = (DevAbs:get(Revn.."Abs:NameBot") or 'ريفن')
local GetJson = '{"BotId": '..Revn..',"BotName": "'..BotName..'","GroupsList":{'  
for k,v in pairs(List) do 
LinkGroups = DevAbs:get(Revn.."Abs:Groups:Links"..v)
Welcomes = DevAbs:get(Revn..'Abs:Groups:Welcomes'..v) or ''
Welcomes = Welcomes:gsub('"',"") Welcomes = Welcomes:gsub("'","") Welcomes = Welcomes:gsub(",","") Welcomes = Welcomes:gsub("*","") Welcomes = Welcomes:gsub(";","") Welcomes = Welcomes:gsub("`","") Welcomes = Welcomes:gsub("{","") Welcomes = Welcomes:gsub("}","") 
AbsConstructors = DevAbs:smembers(Revn..'Abs:AbsConstructor:'..v)
Constructors = DevAbs:smembers(Revn..'Abs:BasicConstructor:'..v)
BasicConstructors = DevAbs:smembers(Revn..'Abs:Constructor:'..v)
Managers = DevAbs:smembers(Revn..'Abs:Managers:'..v)
Admis = DevAbs:smembers(Revn..'Abs:Admins:'..v)
Vips = DevAbs:smembers(Revn..'Abs:VipMem:'..v)
if k == 1 then
GetJson = GetJson..'"'..v..'":{'
else
GetJson = GetJson..',"'..v..'":{'
...
GetJson = GetJson..'}}'
local File = io.open('./'..Revn..'.json', "w")
File:write(GetJson)
File:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..Revn..'.json', '⌔︙يحتوي الملف على ↫ '..#List..' مجموعه',dl_cb, nil)
io.popen('rm -rf ./'..Revn..'.json')
end
...
--     Source Revn     --
if DevAbs:get(Revn.."SET:GAME"..msg.chat_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
Dev_Abs( msg.chat_id_, msg.id_, 1,"⌔︙يوجد فقط ( 6 ) اختيارات\n⌔︙ارسل اختيارك مره اخرى", 1, "md")    
return false  end 
local GETNUM = DevAbs:get(Revn.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(Revn.."SET:GAME"..msg.chat_id_)   
Dev_Abs( msg.chat_id_, msg.id_, 1,'⌔︙*المحيبس باليد رقم* ↫ '..NUM..'\n⌔︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, "md") 
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
...
if DevAbs:get(Revn..'DevAbs4'..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_, "⌔︙تم الغاء الامر")
DevAbs:del(Revn..'DevAbs4'..msg.sender_user_id_)
return false  end 
DevAbs:del(Revn..'DevAbs4'..msg.sender_user_id_)
...
DevAbs:set(Revn..'Abs:ChId',data.id_)
else
...
--     Source Revn     --
if DevAbs:get(Revn.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
DevAbs:del(Revn.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء الامر', 1, 'md')
return false 
end 
DevAbs:del(Revn.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
local DevText = msg.content_.text_:match("(.*)")
DevAbs:set(Revn.."DevText", DevText)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ كليشة المطور", 1, "md")
end
if DevAbs:get(Revn..'Abs:NameBot'..msg.sender_user_id_) == 'msg' then
if text and text:match("^الغاء$") then 
DevAbs:del(Revn..'Abs:NameBot'..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء الامر', 1, 'md')
return false 
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حفظ اسم البوت ', 1, 'html')
DevAbs:del(Revn..'Abs:NameBot'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:NameBot', text)
return false 
end
--     Source Revn     --
if text == "الرابط" then
if not DevAbs:get(Revn.."Abs:Lock:GpLinks"..msg.chat_id_) then 
if DevAbs:get(Revn.."Abs:Groups:Links"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙𝒈𝒓𝒐𝒖𝒑 𝒍𝒊𝒏𝒌 ↬ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"..DevAbs:get(Revn.."Abs:Groups:Links"..msg.chat_id_), 1, "html")
else 
...
NameBot = (DevAbs:get(Revn..'Abs:NameBot') or 'ريفن')
local RevnTEAM = {"لتكول بوت اسمي "..NameBot.." 😒🔪","اسمي القميل "..NameBot.." 😚♥️","عندي اسم تره 😒💔","صيحولي "..NameBot.." كافي بوت 😒🔪","انت البوت لك 😒💔"} 
Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM[math.random(#RevnTEAM)] , 1, 'html') 
return false
end
if text == 'اسم البوت' or text == 'البوت شنو اسمه' or text == 'شسمه البوت' or text == 'البوت شسمه' then
NameBot = (DevAbs:get(Revn..'Abs:NameBot') or 'ريفن') 
local RevnTEAM = {"اسمي القميل "..NameBot.." 😚♥️","هلاا يروحيي وياكك "..NameBot.." 😻♥️"} 
Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM[math.random(#RevnTEAM)] , 1, 'html') 
return false
end
if text and text == (DevAbs:get(Revn..'Abs:NameBot') or 'ريفن') then 
NameBot = (DevAbs:get(Revn..'Abs:NameBot') or 'ريفن')
local RevnTEAM = {'😸♥️ هلا كلبي وياك '..NameBot..' تفضل','ترةه مصختهاا احجيي شرايد 😕😒💔','اطلقق واحدد يصيح '..NameBot..' 😻♥️','خبصتت امنةة شتريدد عااد 🤧😒💔'} 
Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM[math.random(#RevnTEAM)] , 1, 'html') 
return false 
end
if text =='نقاطي' and ChCheck(msg) then 
if tonumber((DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙لم تربح اي نقطه\n⌔︙ارسل ↫ الالعاب للعب', 1, 'md')
else 
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙عدد النقاط التي ربحتها ↫ '..(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)), 1, 'md')
end
end
if text ==  'حذف رسائلي' and ChCheck(msg) or text ==  'مسح رسائلي' and ChCheck(msg) then DevAbs:del(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حذف جميع رسائلك', 1, 'md') end
if text ==  'حذف نقاطي' and ChCheck(msg) or text ==  'مسح نقاطي' and ChCheck(msg) then DevAbs:del(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حذف جميع نقاطك', 1, 'md') end
--     Source Revn     --
if text == 'سمايلات' and ChCheck(msg) or text == 'السمايلات' and ChCheck(msg) then
if not DevAbs:get(Revn..'Abs:Lock:Games'..msg.chat_id_) then
DevAbs2 = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🏆','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🛎','📤','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔️','🛡','🔮','🌡','💣','⏱','🛢','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
name = string.gsub(name,'🎚','🎚')
name = string.gsub(name,'☎️','☎️')
RevnTEAM = '⌔︙اول واحد يدز هذا السمايل يربح ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
RevnTEAM = '⌔︙مبروك لقد ربحت في اللعبه \n⌔︙ارسل ↫ سمايلات للعب مره اخرى'
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
end
DevAbs:set(Revn..'Abs:Games:Ids'..msg.chat_id_,true)
end
...
RevnTEAM = '⌔︙اول واحد يرتبها يربح ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then
...
if not DevAbs:get(Revn..'Abs:Lock:Games'..msg.chat_id_) then
Num = math.random(1,6)
DevAbs:set(Revn.."GAMES"..msg.chat_id_,Num) 
TEST = [[
➀     ➁     ➂     ➃     ➄     ➅
↓     ↓     ↓     ↓     ↓     ↓
👊 ‹› 👊🏻 ‹› 👊🏼 ‹› 👊🏽 ‹› 👊🏾 ‹› 👊🏿
...
DevAbs:setex(Revn.."SET:GAME"..msg.chat_id_, 100, true)  
return false  
end end
...
RevnTEAM = '⌔︙اول واحد يحلها يربح ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum2'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
RevnTEAM = '⌔︙ما معنى هذا السمايل :؟ ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum2'..msg.chat_id_) and not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum3'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
RevnTEAM = '⌔︙ما هو عكس كلمة ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum3'..msg.chat_id_) and not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
...
DevAbs2 = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','📥','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧚‍♂️','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕓','🕤','⌛️','📅','👩‍⚖️','👨‍🎨'};
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum4'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
name = string.gsub(name,'😸','😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠️','💀💀💀☠️💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼')
...
name = string.gsub(name,'👩‍⚖️','👨‍⚖️👨‍⚖️👨‍⚖️👨‍⚖️👨‍⚖️👩‍⚖️👨‍⚖️👨‍⚖️')
name = string.gsub(name,'👨‍🎨','👩‍🎨👩‍🎨👨‍🎨👩‍🎨??‍🎨👩‍🎨👩‍🎨👩‍🎨')
RevnTEAM = '⌔︙اول واحد يطلع المختلف يربح\n{'..name..'} '
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum4'..msg.chat_id_) and not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum5'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
RevnTEAM = '⌔︙اكمل المثال التالي ↫ ['..name..']'
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum5'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(Revn..'Abs:GameNum5'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum6'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
RevnTEAM = '⌔︙اكمل المعادله التاليه ↫ ⤈\n{'..name..'} '
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum6'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(Revn..'Abs:GameNum6'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum7'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
RevnTEAM = '⌔︙ما معنى كلمة ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md')
return false
end end
if text == DevAbs:get(Revn..'Abs:GameNum7'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(Revn..'Abs:GameNum7'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(Revn..'Abs:GameNum8'..msg.chat_id_,name)
DevAbs:del(Revn..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(Revn..'Abs:GameNum8'..msg.chat_id_) then
if not DevAbs:get(Revn..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(Revn..'Abs:GameNum8'..msg.chat_id_)
...
--     Source Revn     --
if DevAbs:get(Revn.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
Dev_Abs(msg.chat_id_, msg.id_, 1,"⌔︙عذرا لا يمكنك تخمين عدد اكبر من الـ20 خمن رقم ما بين الـ1 والـ20", 1, 'md')
return false  end 
local GETNUM = DevAbs:get(Revn.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(Revn..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevAbs:del(Revn.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙*التخمين الصحيح هو* ↫ '..NUM..'\n⌔︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
DevAbs:incrby(Revn..'Set:Num'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(DevAbs:get(Revn..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
DevAbs:del(Revn..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevAbs:del(Revn.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙*التخمين الصحيح هو* ↫ '..GETNUM..'\n⌔︙*للاسف لقد خسرت حاول مره اخرى لتخمين الرقم الصحيح*', 1, 'md')
else
if tonumber(DevAbs:get(Revn..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 1 then
SetNum = 'محاولتان فقط'
elseif tonumber(DevAbs:get(Revn..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 2 then
...
DevAbs:set(Revn.."GAMES:NUM"..msg.chat_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙اهلا بك عزيزي في لعبة التخمين ↫ ⤈\n ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n⌔︙سيتم تخمين عدد ما بين الـ1 والـ20 اذا تعتقد انك تستطيع الفوز جرب واللعب الان .\n⌔︙ملاحظه لديك ثلاث محاولات فقط فكر قبل ارسال تخمينك !', 1, 'md')
DevAbs:setex(Revn.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_, 100, true)  
return false  
end
end
--     Source Revn     --
if text == 'روليت' then
if not DevAbs:get(Revn..'Abs:Lock:Games'..msg.chat_id_) then
DevAbs:del(Revn.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevAbs:del(Revn..'Abs:ListRolet'..msg.chat_id_)  
DevAbs:setex(Revn.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_,3600,true)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙حسنا لنلعب , ارسل عدد اللاعبين للروليت .', 1, 'md')
return false  
end
end
if text and text:match("^(%d+)$") and DevAbs:get(Revn.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_) then
if text == "1" then
Text = "⌔︙لا استطيع بدء اللعبه بلاعب واحد فقط"
else
DevAbs:set(Revn.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_,text)  
Text = '⌔︙تم بدء تسجيل اللسته يرجى ارسال المعرفات \n⌔︙الفائز يحصل على 5 نقاط عدد المطلوبين ↫ '..text..' لاعب'
end
DevAbs:del(Revn.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,Text)
return false
end
if text and text:match('^(@[%a%d_]+)$') and DevAbs:get(Revn.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) then 
if DevAbs:sismember(Revn..'Abs:ListRolet'..msg.chat_id_,text) then
...
DevAbs:sadd(Revn..'Abs:ListRolet'..msg.chat_id_,text)
local CountAdd = DevAbs:get(Revn.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_)
local CountAll = DevAbs:scard(Revn..'Abs:ListRolet'..msg.chat_id_)
local CountUser = CountAdd - CountAll
if tonumber(CountAll) == tonumber(CountAdd) then 
DevAbs:del(Revn.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevAbs:setex(Revn.."Abs:WittingStartRolet"..msg.chat_id_..msg.sender_user_id_,1400,true) 
...
if not DevAbs:get(Revn..'Abs:Lock:Gamesinline'..msg.chat_id_) then
Text =[[
...
{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}},
}
...
DevAbs0 = (DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) * 50)
DevAbs:incrby(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_,DevAbs0)
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙تم بيع '..(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_))..' من نقاطك\n⌔︙كل نقطه تساوي 50 رساله', 'md')
DevAbs:del(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)
end
end
--     Source Revn     --
...
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..msg.chat_id_,Manager_id)  
DevAbs:sadd(Revn..'Abs:AbsConstructor:'..msg.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
if DevAbs:get(Revn.."Abs:Left:Bot"..Revn) and not SecondSudo(msg) then
...
local msguser = tonumber(DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_))
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local cont = (tonumber(DevAbs:get(Revn..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
local user_nkt = tonumber(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
if result.username_ then username = '@'..result.username_ else username = 'لا يوجد' end
...
DevAbs:setex(Revn..'DevAbs4'..msg.sender_user_id_,360,true)
...
function RevnTEAM(extra,result,success)
if DevAbs:get(Revn.."Abs:Send:Bot"..Revn) and not SecondSudo(msg) then 
send(msg.chat_id_, msg.id_,"⌔︙الاذاعه معطله من قبل المطور الاساسي")
return false
end
local GpList = DevAbs:smembers(Revn.."Abs:Groups")
for k,v in pairs(GpList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
local PvList = DevAbs:smembers(Revn.."Abs:Users")
for k,v in pairs(PvList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم اذاعة رسالتك بالتوجيه \n⌔︙‏في ↫ ❨ '..#GpList..' ❩ مجموعه \n⌔︙والى ↫ ❨ '..#PvList..' ❩ مشترك \n ✓', 1, 'md')
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),RevnTEAM)
end
end
--     Source Revn     --
if text == "مشاهده المنشور" and ChCheck(msg) or text == "مشاهدات المنشور" and ChCheck(msg) or text == "عدد المشاهدات" and ChCheck(msg) then
DevAbs:set(Revn..'Abs:viewget'..msg.sender_user_id_,true)
...
⌔︙[Exp Source](https://t.me/XAXRT)

⌔︙[Developer](https://t.me/RFRFF)
┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉
⌔︙[Tws Revn](https://t.me/XXTTAXbot)
]]
...
if DevAbs:get(Revn.."Abs:Kick:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(Revn..'yes'..msg.sender_user_id_, 'delyes')
DevAbs:set(Revn..'no'..msg.sender_user_id_, 'delno')
...
DevAbs:set(Revn.."Abs:Kick:Me"..msg.chat_id_, true)
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل امر اطردني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل اطردني' and Manager(msg) and ChCheck(msg) then
DevAbs:del(Revn.."Abs:Kick:Me"..msg.chat_id_)
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل امر اطردني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
end
--     Source Revn     --
if text == "نزلني" and ChCheck(msg) then
if DevAbs:get(Revn.."Abs:Del:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(Revn..'yesdel'..msg.sender_user_id_, 'delyes')
DevAbs:set(Revn..'nodel'..msg.sender_user_id_, 'delno')
...
DevAbs:set(Revn.."Abs:Del:Me"..msg.chat_id_, true)
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل امر نزلني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل نزلني' and BasicConstructor(msg) and ChCheck(msg) then
DevAbs:del(Revn.."Abs:Del:Me"..msg.chat_id_)
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل امر نزلني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
end
--     Source Revn     --
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:TagAll'..msg.chat_id_)
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:TagAll'..msg.chat_id_,true)
end
if Admin(msg) then
if text == "تاك للكل" and ChCheck(msg) then
if not DevAbs:get(Revn..'Abs:Lock:TagAll'..msg.chat_id_) then
function TagAll(dp1,dp2)
local text = "⌔︙وينكم يالربع \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
i = 0
for k, v in pairs(dp2.members_) do
i = i + 1
if DevAbs:get(Revn..'Save:UserName'..v.user_id_) then
text = text..i.."~ : [@"..DevAbs:get(Revn..'Save:UserName'..v.user_id_).."]\n"
else
...
local EntryNumber = (DevAbs:get(Revn..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local ExitNumber = (DevAbs:get(Revn..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local MsgNumberDay = (DevAbs:get(Revn..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d')) or 0)
...
if text == "جهاتي" and ChCheck(msg) or text == "اضافاتي" and ChCheck(msg) then add = (tonumber(DevAbs:get(Revn..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0) Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙عدد جهاتك المضافه ↫ *❨ "..add.." ❩* ", 1, 'md') end
if text == "تعديلاتي" or text == "سحكاتي" and ChCheck(msg) then local edit_msg = DevAbs:get(Revn..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0  Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙عدد تعديلاتك ↫ *❨ "..edit_msg.." ❩* ", 1, 'md') end
...
if text == 'مسح سحكاتي' or text == 'مسح تعديلاتي' or text == 'حذف سحكاتي' or text == 'حذف تعديلاتي' then DevAbs:del(Revn..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حذف جميع تعديلاتك بنجاح' , 1, 'md') end
if text == 'مسح جهاتي' or text == 'مسح اضافاتي' or text == 'حذف جهاتي' or text == 'حذف اضافاتي' then DevAbs:del(Revn..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حذف جميع جهاتك المضافه' , 1, 'md') end
--     Source Revn     --
if text == "المطور" then 
local DevText = DevAbs:get(Revn.."DevText")
if DevAbs:get(Revn.."Abs:ChId") then local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevAbs:get(Revn.."Abs:ChId")) local GetInfo = JSON.decode(Check) local DevCh1 = GetInfo.result.username DevCh = '\n⌔︙*Dev Ch* ↬ [@'..DevCh1..']' else DevCh = '' end
...
keyboard = {} 
keyboard.inline_keyboard = {{{text=dp.first_name_,url=("t.me/"..dp.username_ or "t.me/HATLLA")}}}
local msg_id = msg.id_/2097152/0.5
...
Text = "• *The developer and programmer of this source is* : [مطور السورس](https://t.me/RFRFF)."
keyboard = {} 
keyboard.inline_keyboard = {{{text='- مطور السورس .',url="t.me/RFRFF"}}}
local msg_id = msg.id_/2097152/0.5
...
if not DevAbs:get(Revn..'Abs:Lock:Stupid'..msg.chat_id_) then
local username = text:match('^هينه @(.*)') or text:match('^هينها @(.*)') 
function RevnTEAM(extra,result,success)
if result.id_ then  
if tonumber(result.id_) == tonumber(Revn) then  
...
if DevAbs:sismember(Revn.."Abs:AbsConstructor:"..msg.chat_id_,result.id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local RevnTEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md') 
local RevnTEAM = { "لكك جرجف @"..username.." احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش لكك فاشل @"..username.." لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","حبيبي @"..username.." راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪","دمشي لك @"..username.." ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع @"..username.." متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪",}
Dev_Abs(msg.chat_id_, result.id_, 1,''..RevnTEAM[math.random(#RevnTEAM)], 1, 'html') 
else  
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو غير موجود في المجموعه', 1, 'md') 
end 
end 
resolve_username(username,RevnTEAM)
end
end
--     Source Revn     --
...
if tonumber(result.sender_user_id_) == tonumber(Revn) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, 'شو تمضرط اكو واحد يهين نفسه؟🤔👌🏿', 1, 'md') 
return false  
end  
...
if DevAbs:sismember(Revn.."Abs:AbsConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local RevnTEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md') 
local RevnTEAM = {"لكك جرجف احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش فاشل لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","دمشي لك ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪","حبيبي راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..RevnTEAM[math.random(#RevnTEAM)], 1, 'md') 
end 
...
local RevnTEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md') 
local RevnTEAM = {"مواححح افيش عافيههه😍🔥💗","امممووااهحح شهلعسل🥺🍯💘","مواححح،ءوفف اذوب🤤💗"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..RevnTEAM[math.random(#RevnTEAM)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local RevnTEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md') 
local RevnTEAM = {"تتعال ححب محتاجيك🙂🍭","تعال يولل استاذكك ايريدككك😒🔪","يمعوود تعاال يريدوكك🤕♥️","تعال لكك ديصيحوك😐🖤"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..RevnTEAM[math.random(#RevnTEAM)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local RevnTEAM = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,RevnTEAM, 1, 'md') 
local RevnTEAM = { "تتعال ححب @"..username.." محتاجيك🙂🍭","تعال يولل @"..username.." استاذكك ايريدككك😒🔪","يمعوود @"..username.." تعاال يريدوكك🤕♥️","تعال لكك @"..username.." ديصيحوك😐🖤",}
Dev_Abs(msg.chat_id_, result.id_, 1,''..RevnTEAM[math.random(#RevnTEAM)], 1, 'html') 
else  
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙العضو غير موجود في المجموعه', 1, 'md') 
end 
...
if DevAbs:sismember(Revn..'Abs:SecondSudo:',result.sender_user_id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(Revn..'Abs:SudoBot:',result.sender_user_id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(Revn..'Abs:ManagerAll:',result.sender_user_id_) then
managerall = 'المدراء العامين • ' else managerall = '' end
if DevAbs:sismember(Revn..'Abs:AdminAll:',result.sender_user_id_) then
adminall = 'الادمنيه العامين • ' else adminall = '' end
if DevAbs:sismember(Revn..'Abs:VipAll:',result.sender_user_id_) then
vpall = 'المميزين العامين • ' else vpall = '' end
if DevAbs:sismember(Revn..'Abs:BasicConstructor:'..msg.chat_id_, result.sender_user_id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(Revn..'Abs:Constructor:'..msg.chat_id_, result.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(Revn..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(Revn..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(Revn..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(Revn..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_) then
...
DevAbs:srem(Revn..'Abs:SecondSudo:', result.sender_user_id_)
DevAbs:srem(Revn..'Abs:SudoBot:', result.sender_user_id_)
DevAbs:srem(Revn..'Abs:ManagerAll:', result.sender_user_id_)
DevAbs:srem(Revn..'Abs:AdminAll:', result.sender_user_id_)
DevAbs:srem(Revn..'Abs:VipAll:', result.sender_user_id_)
DevAbs:srem(Revn..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(Revn..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(Revn..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(Revn..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(Revn..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(Revn..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(Revn..'Abs:SecondSudo:',result.id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(Revn..'Abs:SudoBot:',result.id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(Revn..'Abs:ManagerAll:',result.id_) then
managerall = 'المدراء العامين • ' else managerall = '' end
if DevAbs:sismember(Revn..'Abs:AdminAll:',result.id_) then
adminall = 'الادمنيه العامين • ' else adminall = '' end
if DevAbs:sismember(Revn..'Abs:VipAll:',result.id_) then
vpall = 'المميزين العامين • ' else vpall = '' end
if DevAbs:sismember(Revn..'Abs:BasicConstructor:'..msg.chat_id_, result.id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(Revn..'Abs:Constructor:'..msg.chat_id_, result.id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(Revn..'Abs:Managers:'..msg.chat_id_, result.id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(Revn..'Abs:Admins:'..msg.chat_id_, result.id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(Revn..'Abs:VipMem:'..msg.chat_id_, result.id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(Revn..'Abs:Cleaner:'..msg.chat_id_, result.id_) then
cleaner = 'المنظفين • ' else cleaner = ''
end
...
if AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'sudoid' then
DevAbs:srem(Revn..'Abs:SecondSudo:', result.id_)
DevAbs:srem(Revn..'Abs:SudoBot:', result.id_)
DevAbs:srem(Revn..'Abs:ManagerAll:', result.id_)
DevAbs:srem(Revn..'Abs:AdminAll:', result.id_)
DevAbs:srem(Revn..'Abs:VipAll:', result.id_)
DevAbs:srem(Revn..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
DevAbs:srem(Revn..'Abs:Constructor:'..msg.chat_id_,result.id_)
DevAbs:srem(Revn..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:srem(Revn..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:srem(Revn..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:srem(Revn..'Abs:Cleaner:'..msg.chat_id_, result.id_)
elseif AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'secondsudo' then
DevAbs:srem(Revn..'Abs:SudoBot:', result.id_)
DevAbs:srem(Revn..'Abs:ManagerAll:', result.id_)
DevAbs:srem(Revn..'Abs:AdminAll:', result.id_)
...
DevAbs:sadd(Revn..'Abs:SecondSudo:',result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Revn..'Abs:SecondSudo:',result.id_)
...
DevAbs:sadd(Revn..'Abs:SecondSudo:',user)
...
DevAbs:srem(Revn..'Abs:SecondSudo:',user)
...
function sudo_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:SudoBot:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:SudoBot:',result.id_)
...
DevAbs:sadd(Revn..'Abs:SudoBot:',user)
...
DevAbs:srem(Revn..'Abs:SudoBot:',user)
...
DevAbs:sadd(Revn..'Abs:ManagerAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:ManagerAll:',result.id_)
...
DevAbs:sadd(Revn..'Abs:ManagerAll:',user)
...
DevAbs:srem(Revn..'Abs:ManagerAll:',user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:AdminAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:AdminAll:',result.id_)
...
DevAbs:sadd(Revn..'Abs:AdminAll:',user)
...
DevAbs:srem(Revn..'Abs:AdminAll:',user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:VipAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:VipAll:',result.id_)
...
DevAbs:sadd(Revn..'Abs:VipAll:',user)
...
DevAbs:srem(Revn..'Abs:VipAll:',user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:AbsConstructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:AbsConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Revn..'Abs:AbsConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:AbsConstructor:'..msg.chat_id_,result.sender_user_id_)
...
DevAbs:srem(Revn..'Abs:AbsConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:srem(Revn..'Abs:AbsConstructor:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Revn..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:Constructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Revn..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:sadd(Revn..'Abs:Managers:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:Managers:'..msg.chat_id_,user)
...
function prom_reply(extra, result, success)
DevAbs:sadd(Revn..'Abs:Cleaner:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Revn..'Abs:Cleaner:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Revn..'Abs:Cleaner:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:Cleaner:'..msg.chat_id_,user)
...
function prom_reply(extra, result, success)
if not BasicConstructor(msg) and DevAbs:get(Revn.."Abs:Lock:ProSet"..msg.chat_id_) then 
...
DevAbs:sadd(Revn..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:sadd(Revn..'Abs:VipMem:'..msg.chat_id_,user)
...
DevAbs:srem(Revn..'Abs:VipMem:'..msg.chat_id_,user)
...
local Text = '⌔︙قم باستعمال الازرار لرفع العضو ↫ ⤈\n⌔︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'HATLLA')..')'
if Sudo(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text="رفع منشئ",callback_data=msg.sender_user_id_..":SetConstructor:"..result.sender_user_id_},{text="رفع مدير",callback_data=msg.sender_user_id_..":SetManager:"..result.sender_user_id_}},{{text="رفع منظف",callback_data=msg.sender_user_id_..":SetCleaner:"..result.sender_user_id_},{text="رفع منشئ اساسي",callback_data=msg.sender_user_id_..":SetBasicConstructor:"..result.sender_user_id_}},{{text="رفع مالك",callback_data=msg.sender_user_id_..":SetAbsConstructor:"..result.sender_user_id_},{text="رفع مطور",callback_data=msg.sender_user_id_..":SetSudoBot:"..result.sender_user_id_}},{{text="رفع مطور ثانوي",callback_data=msg.sender_user_id_..":SetSecondSudo:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif SecondSudo(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text="رفع منشئ",callback_data=msg.sender_user_id_..":SetConstructor:"..result.sender_user_id_},{text="رفع مدير",callback_data=msg.sender_user_id_..":SetManager:"..result.sender_user_id_}},{{text="رفع منظف",callback_data=msg.sender_user_id_..":SetCleaner:"..result.sender_user_id_},{text="رفع منشئ اساسي",callback_data=msg.sender_user_id_..":SetBasicConstructor:"..result.sender_user_id_}},{{text="رفع مالك",callback_data=msg.sender_user_id_..":SetAbsConstructor:"..result.sender_user_id_},{text="رفع مطور",callback_data=msg.sender_user_id_..":SetSudoBot:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif SudoBot(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text="رفع منشئ",callback_data=msg.sender_user_id_..":SetConstructor:"..result.sender_user_id_},{text="رفع مدير",callback_data=msg.sender_user_id_..":SetManager:"..result.sender_user_id_}},{{text="رفع منظف",callback_data=msg.sender_user_id_..":SetCleaner:"..result.sender_user_id_},{text="رفع منشئ اساسي",callback_data=msg.sender_user_id_..":SetBasicConstructor:"..result.sender_user_id_}},{{text="رفع مالك",callback_data=msg.sender_user_id_..":SetAbsConstructor:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif AbsConstructor(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text="رفع منشئ",callback_data=msg.sender_user_id_..":SetConstructor:"..result.sender_user_id_},{text="رفع مدير",callback_data=msg.sender_user_id_..":SetManager:"..result.sender_user_id_}},{{text="رفع منظف",callback_data=msg.sender_user_id_..":SetCleaner:"..result.sender_user_id_},{text="رفع منشئ اساسي",callback_data=msg.sender_user_id_..":SetBasicConstructor:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif BasicConstructor(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text="رفع منشئ",callback_data=msg.sender_user_id_..":SetConstructor:"..result.sender_user_id_},{text="رفع مدير",callback_data=msg.sender_user_id_..":SetManager:"..result.sender_user_id_}},{{text="رفع منظف",callback_data=msg.sender_user_id_..":SetCleaner:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Constructor(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text="رفع منظف",callback_data=msg.sender_user_id_..":SetCleaner:"..result.sender_user_id_},{text="رفع مدير",callback_data=msg.sender_user_id_..":SetManager:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Manager(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_},{text="رفع ادمن",callback_data=msg.sender_user_id_..":SetAdmin:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Admin(msg) then
inline = {{{text="رفع مميز",callback_data=msg.sender_user_id_..":SetMem:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
end
...
local Text = '⌔︙قم باستعمال الازرار لتنزيل العضو ↫ ⤈\n⌔︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'HATLLA')..')'
if Sudo(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text="تنزيل منشئ",callback_data=msg.sender_user_id_..":RemConstructor:"..result.sender_user_id_},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..result.sender_user_id_}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..result.sender_user_id_},{text="تنزيل منشئ اساسي",callback_data=msg.sender_user_id_..":RemBasicConstructor:"..result.sender_user_id_}},{{text="تنزيل مالك",callback_data=msg.sender_user_id_..":RemAbsConstructor:"..result.sender_user_id_},{text="تنزيل مطور",callback_data=msg.sender_user_id_..":RemSudoBot:"..result.sender_user_id_}},{{text="تنزيل مطور ثانوي",callback_data=msg.sender_user_id_..":RemSecondSudo:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif SecondSudo(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text="تنزيل منشئ",callback_data=msg.sender_user_id_..":RemConstructor:"..result.sender_user_id_},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..result.sender_user_id_}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..result.sender_user_id_},{text="تنزيل منشئ اساسي",callback_data=msg.sender_user_id_..":RemBasicConstructor:"..result.sender_user_id_}},{{text="تنزيل مالك",callback_data=msg.sender_user_id_..":RemAbsConstructor:"..result.sender_user_id_},{text="تنزيل مطور",callback_data=msg.sender_user_id_..":RemSudoBot:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif SudoBot(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text="تنزيل منشئ",callback_data=msg.sender_user_id_..":RemConstructor:"..result.sender_user_id_},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..result.sender_user_id_}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..result.sender_user_id_},{text="تنزيل منشئ اساسي",callback_data=msg.sender_user_id_..":RemBasicConstructor:"..result.sender_user_id_}},{{text="تنزيل مالك",callback_data=msg.sender_user_id_..":RemAbsConstructor:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif AbsConstructor(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text="تنزيل منشئ",callback_data=msg.sender_user_id_..":RemConstructor:"..result.sender_user_id_},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..result.sender_user_id_}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..result.sender_user_id_},{text="تنزيل منشئ اساسي",callback_data=msg.sender_user_id_..":RemBasicConstructor:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif BasicConstructor(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text="تنزيل منشئ",callback_data=msg.sender_user_id_..":RemConstructor:"..result.sender_user_id_},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..result.sender_user_id_}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Constructor(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..result.sender_user_id_},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Manager(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
elseif Admin(msg) then
inline = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..result.sender_user_id_}},{{text = '⌁ Revn TeAM .',url="t.me/HATLLA"}}}
end
SendInline(msg.chat_id_,Text,nil,inline,msg.id_/2097152/0.5)
end,nil)
end 
...
function promote_by_reply(extra, result, success)
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChatMember?chat_id='..msg.chat_id_..'&user_id='..Revn)
local GetInfo = JSON.decode(Check)
...
if text == "راسلني" then
RevnTEAM = {"ها هلاو","انطق","كول حبي","تفضل"};
send(msg.sender_user_id_, 0,RevnTEAM[math.random(#RevnTEAM)])
end
--     Source Revn     --
...
DevAbs:del(Revn.."Abs:Lock:ProSet"..msg.chat_id_)
...
DevAbs:set(Revn.."Abs:Lock:ProSet"..msg.chat_id_,"true")
...
if not Constructor(msg) and DevAbs:get(Revn.."Abs:Lock:KickBan"..msg.chat_id_) then 
...
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(Revn..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(Revn..'Abs:Ban:'..msg.chat_id_, result.id_)
...
ChatKick(msg.chat_id_, user)
DevAbs:sadd(Revn..'Abs:Ban:'..msg.chat_id_, user)
...
DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_, result.id_)
...
DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_, user)
...
if not Constructor(msg) and DevAbs:get(Revn.."Abs:Lock:MuteTked"..msg.chat_id_) then 
...
if DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","⌔︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(Revn..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","⌔︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(Revn..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","⌔︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(Revn..'Abs:Muted:'..msg.chat_id_, user)
...
if not DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","⌔︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
if not DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","⌔︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if not DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","⌔︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_, user)
...
DevAbs:sadd(Revn..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:sadd(Revn..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:sadd(Revn..'Abs:Tkeed:'..msg.chat_id_, user)
...
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_, user)
...
if DevAbs:sismember(Revn..'Abs:SecondSudo:',result.sender_user_id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙*لاتستطيع حظر المطور الثانوي*", 1, 'md')
return false 
end
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(Revn..'Abs:BanAll:', result.sender_user_id_)
...
if DevAbs:sismember(Revn..'Abs:SecondSudo:',result.id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙*لاتستطيع حظر المطور الثانوي*", 1, 'md')
return false 
end
if result.id_ then
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(Revn..'Abs:BanAll:', result.id_)
...
if tonumber(user) == tonumber(Revn) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙*لاتستطيع حظر البوت عام*", 1, 'md')
return false 
end
...
if DevAbs:sismember(Revn..'Abs:SecondSudo:',user) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙*لاتستطيع حظر المطور الثانوي*", 1, 'md')
return false 
end
ChatKick(msg.chat_id_, user)
DevAbs:sadd(Revn..'Abs:BanAll:', user)
...
DevAbs:sadd(Revn..'Abs:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Revn..'Abs:MuteAll:', result.id_)
...
DevAbs:sadd(Revn..'Abs:MuteAll:', user)
...
DevAbs:srem(Revn..'Abs:BanAll:', result.sender_user_id_)
DevAbs:srem(Revn..'Abs:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(Revn..'Abs:BanAll:', result.id_)
DevAbs:srem(Revn..'Abs:MuteAll:', result.id_)
...
DevAbs:srem(Revn..'Abs:BanAll:', user)
DevAbs:srem(Revn..'Abs:MuteAll:', user)
...
DevAbs:setex(Revn.."Abs:EditDev"..msg.sender_user_id_,300,true)
end
if DevAbs:get(Revn.."Abs:EditDev"..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_,'⌔︙تم الغاء امر تغير المطور الاساسي')
DevAbs:del(Revn.."Abs:EditDev"..msg.sender_user_id_)
return false
end
if text and text:match("^(%d+)$") then 
...
if dp.first_name_ ~= false then
DevAbs:del(Revn.."Abs:EditDev"..msg.sender_user_id_)
DevAbs:set(Revn.."Abs:NewDev"..msg.sender_user_id_,dp.id_)
...
--     Source Revn     --
if msg.reply_to_message_id_ ~= 0 then
if text and text:match("^رفع مطي$") and not DevAbs:get(Revn..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if DevAbs:sismember(Revn..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","⌔︙هو مطي شرفع منه بعد😹💔") 
else
ReplyStatus(msg,result.sender_user_id_,"Reply","⌔︙تم رفعه في قائمة المطايه") 
DevAbs:sadd(Revn..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
end end
getMessage(msg.chat_id_, msg.reply_to_message_id_,donky_by_reply)
end end
--     Source Revn     --
if msg.reply_to_message_id_ ~= 0  then
if text and text:match("^تنزيل مطي$") and not DevAbs:get(Revn..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if not DevAbs:sismember(Revn..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","⌔︙هو ليس مطي ليتم تنزيله") 
else
DevAbs:srem(Revn..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:set('RevnTEAM:'..Revn..'id:user'..msg.chat_id_,TXT)  
DevAbs:setex('RevnTEAM:'..Revn.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
DevAbs:del(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_) 
DevAbs:incrby(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_,Num) 
...
DevAbs:set('RevnTEAM:'..Revn..'ids:user'..msg.chat_id_,TXT)  
DevAbs:setex('RevnTEAM:'..Revn.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
function Reply(extra, result, success)
DevAbs:incrby(Revn..'Abs:GamesNumber'..msg.chat_id_..result.sender_user_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم اضافة "..Num..' نقطه', 1, 'md') 
end
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},Reply, nil)
return false
end
if DevAbs:get(Revn..'Abs:Lock:Clean'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(Revn.."Abs:cleaner"..msg.chat_id_, msg.id_) else DevAbs:sadd(Revn.."Abs:cleaner"..msg.chat_id_, msg.id_) end end end
if Manager(msg) and msg.reply_to_message_id_ ~= 0 then
if text and text:match("^تثبيت$") and ChCheck(msg) then 
if DevAbs:sismember(Revn.."Abs:Lock:Pinpin",msg.chat_id_) and not BasicConstructor(msg) then
...
if data.ID == "Ok" then
DevAbs:set(Revn..'Abs:PinnedMsg'..msg.chat_id_,msg.reply_to_message_id_)
...
local List = DevAbs:smembers(Revn..'Abs:VipMem:'..msg.chat_id_)
text = "⌔︙قائمة المميزين ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..abs)
text = "⌔︙قائمة الادمنيه ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:Managers:'..msg.chat_id_)
text = "⌔︙قائمة المدراء ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:Cleaner:'..msg.chat_id_)
text = "⌔︙قائمة المنظفين ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:Constructor:'..msg.chat_id_)
text = "⌔︙قائمة المنشئين ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:AbsConstructor:'..msg.chat_id_)
text = "⌔︙قائمة المالكين ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:BasicConstructor:'..msg.chat_id_)
...
local List = DevAbs:smembers(Revn..'Abs:Muted:'..msg.chat_id_)
...
local List = DevAbs:smembers(Revn..'Abs:Tkeed:'..msg.chat_id_)
text = "⌔︙قائمة المقيدين ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:Ban:'..msg.chat_id_)
...
local List = DevAbs:hkeys(Revn..'Abs:Filters:'..msg.chat_id_)
text = "⌔︙قائمة المنع ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k, v in pairs(List) do
text = text..k..'~ ❨ '..v..' ❩\n'
end
...
local List = DevAbs:smembers(Revn..'User:Donky:'..msg.chat_id_)
...
local List = DevAbs:smembers(Revn..'Abs:SecondSudo:')
...
local BanAll = DevAbs:smembers(Revn..'Abs:BanAll:')
local MuteAll = DevAbs:smembers(Revn..'Abs:MuteAll:')
if #BanAll ~= 0 then 
...
local List = DevAbs:smembers(Revn..'Abs:SudoBot:')
text = "⌔︙قائمة المطورين ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local sudouser = DevAbs:get(Revn..'Abs:Sudos'..v) 
local username = DevAbs:get(Revn..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Revn..'Abs:ManagerAll:')
...
local List = DevAbs:smembers(Revn..'Abs:VipAll:')
...
DevAbs:sadd(Revn.."Abs:AbsConstructor:"..msg.chat_id_,dp.id_)
end,nil)   
end,nil)   
end
end 
...
if result.content_.sticker_ then
local idsticker = result.content_.sticker_.sticker_.persistent_id_
DevAbs:srem(Revn.."Abs:FilterSteckr"..msg.chat_id_,idsticker)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء منع الملصق يمكنهم ارساله الان', 1, 'md')
return false
end
if result.content_.ID == "MessagePhoto" then
local photo = result.content_.photo_.id_
DevAbs:srem(Revn.."Abs:FilterPhoto"..msg.chat_id_,photo)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء منع الصوره يمكنهم ارسالها الان', 1, 'md')
return false
end
if result.content_.animation_.animation_ then
local idanimation = result.content_.animation_.animation_.persistent_id_
DevAbs:srem(Revn.."Abs:FilterAnimation"..msg.chat_id_,idanimation)
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Thwel:Abs'..msg.chat_id_) 
end
if text and (text == "تعطيل تحويل الصيغ" or text == "تعطيل التحويل") and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل تحويل الصيغ'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Thwel:Abs'..msg.chat_id_,true)  
end
if text == 'تحويل' and not DevAbs:get(Revn..'Abs:Thwel:Abs'..msg.chat_id_) and ChCheck(msg) then  
if tonumber(msg.reply_to_message_id_) > 0 then 
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..data.id_) or 0
local user_nkt = tonumber(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..data.id_) or 0)
if DevAbs:sismember(Revn..'Abs:BanAll:',result.sender_user_id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(Revn..'Abs:MuteAll:',result.sender_user_id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(Revn..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(Revn..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(Revn..'Abs:SudoBot:',result.sender_user_id_) and SudoBot(msg) then
sudobot = '\n⌔︙عدد الكروبات ↫ '..(DevAbs:get(Revn..'Abs:Sudos'..result.sender_user_id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..res.id_) or 0
local user_nkt = tonumber(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..res.id_) or 0)
if DevAbs:sismember(Revn..'Abs:BanAll:',res.id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(Revn..'Abs:MuteAll:',res.id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(Revn..'Abs:Ban:'..msg.chat_id_,res.id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_,res.id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(Revn..'Abs:Tkeed:'..msg.chat_id_,res.id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(Revn..'Abs:SudoBot:',res.id_) and SudoBot(msg) then
sudobot = '\n⌔︙عدد الكروبات ↫ '..(DevAbs:get(Revn..'Abs:Sudos'..res.id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..iduser) or 0
local user_nkt = tonumber(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..iduser) or 0)
if DevAbs:sismember(Revn..'Abs:BanAll:',iduser) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(Revn..'Abs:MuteAll:',iduser) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(Revn..'Abs:Ban:'..msg.chat_id_,iduser) then
Tkeed = 'محظور'
elseif DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_,iduser) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(Revn..'Abs:Tkeed:'..msg.chat_id_,iduser) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(Revn..'Abs:SudoBot:',iduser) and SudoBot(msg) then
sudobot = '\n⌔︙عدد الكروبات ↫ '..(DevAbs:get(Revn..'Abs:Sudos'..iduser) or 0)..'' 
else 
sudobot = '' 
end
...
if DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(Revn..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(Revn..'Abs:BanAll:',result.sender_user_id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(Revn..'Abs:MuteAll:',result.sender_user_id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(Revn..'Abs:Tkeed:',result.sender_user_id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
...
if result.id_ then
if DevAbs:sismember(Revn..'Abs:Muted:'..msg.chat_id_,result.id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(Revn..'Abs:Ban:'..msg.chat_id_,result.id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(Revn..'Abs:BanAll:',result.id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(Revn..'Abs:MuteAll:',result.id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(Revn..'Abs:Tkeed:',result.id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙الحظر العام ↫ '..banall..'\n⌔︙الكتم العام ↫ '..muteall..'\n⌔︙الحظر ↫ '..banned..'\n⌔︙الكتم ↫ '..muted..'\n⌔︙التقيد ↫ '..tkeed, 1, 'md')  
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙*المعرف غير صحيح*', 1, 'md')  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Revn..'Abs:BanAll:',result.sender_user_id_) DevAbs:srem(Revn..'Abs:MuteAll:',result.sender_user_id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) 
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),unbanreply) 
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_,user) DevAbs:srem(Revn..'Abs:BanAll:',user) DevAbs:srem(Revn..'Abs:MuteAll:',user)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_,user) 
end  
end,nil)  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_,result.id_) DevAbs:srem(Revn..'Abs:BanAll:',result.id_) DevAbs:srem(Revn..'Abs:MuteAll:',result.id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(Revn..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(Revn..'Abs:Muted:'..msg.chat_id_,result.id_) 
end
end,nil)   
end  
...
DevAbs:set(Revn.."Abs:GpIds:Text"..msg.chat_id_,Text_Rand)
...
DevAbs:set("Revn:New:id:"..Revn..msg.sender_user_id_,'RevnTEAM')
return "RevnTEAM"
end
if text and DevAbs:get("Revn:New:id:"..Revn..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("Revn:New:id:"..Revn..msg.sender_user_id_)
return false
end
DevAbs:del("Revn:New:id:"..Revn..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حفظ كليشة الايدي العامه', 1, 'md')
DevAbs:set(Revn.."Abs:AllIds:Text",text)
return false
end
...
DevAbs:del(Revn.."Abs:AllIds:Text")
end
end
--     Source Revn     --
if text and text:match("^تعيين الايدي$") and ChCheck(msg) or text and text:match("^تعين الايدي$") and ChCheck(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙رجائا اتبع التعليمات للتعيين \n⌔︙لطبع كليشة الايدي ارسل كليشه تحتوي على النصوص التي باللغه الانجليزيه ادناه ↫ ⤈\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n `#username` ↬ لطبع المعرف\n `#id` ↬ لطبع الايدي \n `#photos` ↬ لطبع عدد الصور \n `#stast` ↬ لطبع الرتب \n `#msgs` ↬ لطبع عدد الرسائل \n `#msgday` ↬ لطبع الرسائل اليوميه \n `#CustomTitle` ↬ لطبع اللقب \n `#bio` ↬ لطبع البايو \n `#auto` ↬ لطبع التفاعل \n `#game` ↬ لطبع عدد النقاط \n `#cont` ↬ لطبع عدد الجهات \n `#edit` ↬ لطبع عدد السحكات \n `#Description` ↬ لطبع تعليق الصور\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉', 1, 'md')
DevAbs:set("Revn:New:id:"..Revn..msg.chat_id_..msg.sender_user_id_,'RevnTEAM')
return "RevnTEAM"
end
if text and Manager(msg) and DevAbs:get("Revn:New:id:"..Revn..msg.chat_id_..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("Revn:New:id:"..Revn..msg.chat_id_..msg.sender_user_id_)
return false
end
DevAbs:del("Revn:New:id:"..Revn..msg.chat_id_..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم حفظ الكليشه الجديده', 1, 'md')
DevAbs:set(Revn.."Abs:GpIds:Text"..msg.chat_id_,text)
return false
end
if text and text:match("^حذف الايدي$") and ChCheck(msg) or text and text:match("^مسح الايدي$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم حذف كليشة الايدي")  
DevAbs:del(Revn.."Abs:GpIds:Text"..msg.chat_id_)
end
end
--     Source Revn     --
if msg.reply_to_message_id_ ~= 0 then
...
function RevnTEAM(extra,abbas,success)
...
local msgsday = DevAbs:get(Revn..'Abs:UsersMsgs'..Revn..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit_msg = DevAbs:get(Revn..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(Revn..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local user_nkt = tonumber(DevAbs:get(Revn..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
local cont = (tonumber(DevAbs:get(Revn..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
if abbas.photos_[0] then
if not DevAbs:get(Revn..'Abs:Lock:Id'..msg.chat_id_) then 
if not DevAbs:get(Revn..'Abs:Lock:Id:Photo'..msg.chat_id_) then 
if DevAbs:get(Revn.."Abs:AllIds:Text") then
newpicid = DevAbs:get(Revn.."Abs:AllIds:Text")
...
if not DevAbs:get(Revn.."Abs:GpIds:Text"..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, abbas.photos_[0].sizes_[1].photo_.persistent_id_,newpicid,msg.id_,msg.id_.."")
else 
local new_id = DevAbs:get(Revn.."Abs:GpIds:Text"..msg.chat_id_)
...
if DevAbs:get(Revn.."Abs:AllIds:Text") then
newallid = DevAbs:get(Revn.."Abs:AllIds:Text")
...
if DevAbs:get(Revn.."Abs:AllIds:Text") then
notpicid = DevAbs:get(Revn.."Abs:AllIds:Text")
...
if not DevAbs:get(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم قفل التعديل")  
DevAbs:set(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_,true)
else
...
DevAbs:set(Revn.."Abs:Lock:Bots"..msg.chat_id_,"ked")  
end
if LockText[2] == "التكرار" or LockText[2] == "التكرار بالحذف" then 
DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","del")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم قفل التكرار بالحذف")  
end
if LockText[2] == "التكرار بالطرد" then 
DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","kick")  
...
DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم قفل التكرار بالتقيد")  
end
if LockText[2] == "التكرار بالكتم" then 
DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","mute")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم قفل التكرار بالكتم")  
end
if BasicConstructor(msg) then
if LockText[2] == "التثبيت" then
if not DevAbs:get(Revn..'Abs:Lock:Pin'..msg.chat_id_) then
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  DevAbs:set(Revn.."Abs:PinnedMsg"..msg.chat_id_,data.pinned_message_id_)  end,nil)
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم قفل التثبيت")  
DevAbs:set(Revn..'Abs:Lock:Pin'..msg.chat_id_,true)
DevAbs:sadd(Revn.."Abs:Lock:Pinpin",msg.chat_id_) 
else
...
DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam" ,TextSpam) 
end
end
if text and (text:match("^ضع زمن التكرار (%d+)$") or text:match("^وضع زمن التكرار (%d+)$")) then  
local TextSpam = text:match("ضع زمن التكرار (%d+)$") or text:match("وضع زمن التكرار (%d+)$")
DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam:Time" ,TextSpam) 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:Id:Photo'..msg.chat_id_)
end end
if text and text == 'تعطيل الايدي بالصوره' and ChCheck(msg) then
if DevAbs:get(Revn..'Abs:Lock:Id:Photo'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙الايدي بالصوره بالتاكيد معطل', 1, 'md')
else
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الايدي بالصوره'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:Id:Photo'..msg.chat_id_,true)
end end 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:Id'..msg.chat_id_)
end end 
if text and text == 'تعطيل الايدي' and ChCheck(msg) then
if DevAbs:get(Revn..'Abs:Lock:Id'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙الايدي بالتاكيد معطل ', 1, 'md')
else
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الايدي بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:Id'..msg.chat_id_,true)
end end
end
--     Source Revn     --
...
DevAbs:setex(Revn.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_,300,true) 
end
if text == 'انشاء رابط' or text == 'انشاء الرابط' then
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_))
if not DevAbs:get(Revn.."Abs:Groups:Links"..msg.chat_id_)  then 
if LinkGp.ok == true then 
LinkGroup = LinkGp.result
DevAbs:set(Revn.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn.."Abs:Lock:Welcome"..msg.chat_id_,true)
end
if text and text:match("^تعطيل الترحيب$") and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الترحيب بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn.."Abs:Lock:Welcome"..msg.chat_id_)
end
if DevAbs:get(Revn..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ كليشة الترحيب', 1, 'md')
DevAbs:del(Revn..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(Revn..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
Welcomes = text:gsub('"',"") Welcomes = text:gsub("'","") Welcomes = text:gsub(",","") Welcomes = text:gsub("*","") Welcomes = text:gsub(";","") Welcomes = text:gsub("`","") Welcomes = text:gsub("{","") Welcomes = text:gsub("}","") 
DevAbs:set(Revn..'Abs:Groups:Welcomes'..msg.chat_id_,Welcomes)
...
DevAbs:set(Revn..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
if text and text:match("^حذف الترحيب$") and ChCheck(msg) or text and text:match("^حذف ترحيب$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم حذف الترحيب")  
DevAbs:del(Revn..'Abs:Groups:Welcomes'..msg.chat_id_)
end
if text and text:match("^جلب الترحيب$") and ChCheck(msg) or text and text:match("^جلب ترحيب$") and ChCheck(msg) or text and text:match("^الترحيب$") and ChCheck(msg) then
local Welcomes = DevAbs:get(Revn..'Abs:Groups:Welcomes'..msg.chat_id_)
if Welcomes then
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
else
...
--     Source Revn     --
if DevAbs:get(Revn..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_) then  
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء حفظ الوصف", 1, 'md')
DevAbs:del(Revn..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(Revn..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(Revn..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
DevAbs:set(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"add")  
return false  
end    
if DevAbs:get(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "add" then
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر المنع', 1, 'md')
DevAbs:del(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم منع الكلمه ↫ "..text, 1, 'html')
DevAbs:del(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
DevAbs:hset(Revn..'Abs:Filters:'..msg.chat_id_, text,'newword')
return false
end
if text and text == "الغاء منع" and msg.reply_to_message_id_ == 0 and ChCheck(msg) then       
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙ارسل لي الكلمه الان", 1, 'md') 
DevAbs:set(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"del")  
return false  
end    
if DevAbs:get(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "del" then   
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر الغاء المنع', 1, 'md')
DevAbs:del(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
if not DevAbs:hget(Revn..'Abs:Filters:'..msg.chat_id_, text) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙الكلمه ↫ "..text.." غير ممنوعه", 1, 'html')
DevAbs:del(Revn.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
else
DevAbs:hdel(Revn..'Abs:Filters:'..msg.chat_id_, text)
...
local gps = DevAbs:scard(Revn.."Abs:Groups") local users = DevAbs:scard(Revn.."Abs:Users") 
...
local users = DevAbs:scard(Revn.."Abs:Users")
...
local gps = DevAbs:scard(Revn.."Abs:Groups")
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙عدد المجموعات ↫ ❨ '..gps..' ❩', 1, 'md')
end
end
--     Source Revn     --
if text and text:match('^تنظيف (%d+)$') and ChCheck(msg) then  
if not DevAbs:get(Revn..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_) then  
...
DevAbs:setex(Revn..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_,300,true)
end 
end
if text == "تنظيف المشتركين" and SecondSudo(msg) and ChCheck(msg) then 
local pv = DevAbs:smembers(Revn.."Abs:Users")
local sendok = 0
for i = 1, #pv do
...
DevAbs:srem(Revn.."Abs:Users",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = DevAbs:smembers(Revn.."Abs:Groups")
local w = 0
local q = 0
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
DevAbs:srem(Revn.."Abs:Groups",group[i]) 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = group[i], user_id_ = Revn, status_ = { ID = "ChatMemberStatusLeft" }, }, dl_cb, nil)
w = w + 1
end
...
Dev_Abs(msg.chat_id_, msg.id_, 1,'⌔︙عدد الكروبات الان ↫ { '..#group..' }'..Revngp1..Revngp2..'\n⌔︙العدد الحقيقي الان  ↫ ( '..Revngp3..' ) \n ', 1, 'md')
end end
end,nil)
end
return false
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:Clean'..msg.chat_id_,true)  
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:Clean'..msg.chat_id_) 
end
...
DevAbs:set(Revn..'Abs:CleanNum'..msg.chat_id_,Num) 
end end 
if msg and DevAbs:get(Revn..'Abs:Lock:Clean'..msg.chat_id_) then
if DevAbs:get(Revn..'Abs:CleanNum'..msg.chat_id_) then CleanNum = DevAbs:get(Revn..'Abs:CleanNum'..msg.chat_id_) else CleanNum = 200 end
if DevAbs:scard(Revn.."Abs:cleaner"..msg.chat_id_) >= tonumber(CleanNum) then 
local List = DevAbs:smembers(Revn.."Abs:cleaner"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
...
DevAbs:del(Revn.."Abs:cleaner"..msg.chat_id_)
end 
end 
if Cleaner(msg) then
if DevAbs:get(Revn..'Abs:Lock:Clean'..msg.chat_id_) then 
if text == "الميديا" and ChCheck(msg) or text == "عدد الميديا" and ChCheck(msg) then 
local M = DevAbs:scard(Revn.."Abs:cleaner"..msg.chat_id_)
if M ~= 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙عدد الميديا ↫ "..M.."\n⌔︙الحذف التلقائي ↫ "..(DevAbs:get(Revn..'Abs:CleanNum'..msg.chat_id_) or 200), 1, 'md') 
else
...
if DevAbs:get(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم فتح التعديل")  
DevAbs:del(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_)
else
...
DevAbs:del(Revn.."Abs:Lock:Bots"..msg.chat_id_)  
end
if UnLockText[2] == "التكرار" then 
DevAbs:hdel(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم فتح التكرار")  
end
if BasicConstructor(msg) then
if UnLockText[2] == "التثبيت" then
if DevAbs:get(Revn..'Abs:Lock:Pin'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم فتح التثبيت")  
DevAbs:del(Revn..'Abs:Lock:Pin'..msg.chat_id_)
DevAbs:srem(Revn.."Abs:Lock:Pinpin",msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Text'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Photo'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙الصور بالفعل مفتوحه في المجموعه', 1, 'md')
end
end
if UnLockText[2] == "الكلايش" then
if DevAbs:get(Revn..'Abs:Lock:Spam'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم فتح الكلايش")  
DevAbs:del(Revn..'Abs:Lock:Spam'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Join'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Videos'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Document'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Inline'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Markdown'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Gifs'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Music'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Voice'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Links'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Location'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Tags'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Hashtak'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Contact'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:WebLinks'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Arabic'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:English'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:TagServr'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Stickers'..msg.chat_id_)
else
...
DevAbs:del(Revn..'Abs:Lock:Forwards'..msg.chat_id_)
else
...
DevAbs:set(Revn.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:Links','Abs:Lock:Contact','Abs:Lock:Forwards','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:EditMsgs','Abs:Lock:Stickers','Abs:Lock:Farsi','Abs:Lock:Spam','Abs:Lock:WebLinks','Abs:Lock:Photo'}
for i,Lock in pairs(LockList) do
DevAbs:set(Revn..Lock..msg.chat_id_,true)
end
...
DevAbs:del(Revn..UnLock..msg.chat_id_)
end
...
DevAbs:del(Revn..'Abs:Lock:Fshar'..msg.chat_id_) DevAbs:del(Revn..'Abs:Lock:Taf'..msg.chat_id_) DevAbs:del(Revn..'Abs:Lock:Kfr'..msg.chat_id_) 
DevAbs:set(Revn.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:EditMsgs','Abs:Lock:Farsi','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,Lock in pairs(LockList) do
DevAbs:set(Revn..Lock..msg.chat_id_,true)
...
DevAbs:set(Revn..'Abs:Lock:Fshar'..msg.chat_id_,true) DevAbs:set(Revn..'Abs:Lock:Taf'..msg.chat_id_,true) DevAbs:set(Revn..'Abs:Lock:Kfr'..msg.chat_id_,true) DevAbs:hdel(Revn.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User") 
UnLockList ={'Abs:Lock:EditMsgs','Abs:Lock:Text','Abs:Lock:Arabic','Abs:Lock:English','Abs:Lock:Join','Abs:Lock:Bots','Abs:Lock:Farsi','Abs:Lock:FarsiBan','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,UnLock in pairs(UnLockList) do
DevAbs:del(Revn..UnLock..msg.chat_id_)
end
...
DevAbs:set(Revn..'Abs:Spam:Text'..msg.chat_id_,SetSpam)
...
DevAbs:del(Revn.."Abs:mem:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:Cleaner:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:VipMem:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:Admins:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:Managers:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:Constructor:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:BasicConstructor:Rd"..msg.chat_id_)
DevAbs:del(Revn.."Abs:SudoBot:Rd"..msg.chat_id_)
end
end
--     Source Revn     --
...
if tonumber(admins[i].user_id_) ~= tonumber(Revn) then
...
DevAbs:del(Revn..'Abs:ManagerAll:')
end
if txts[2] == 'المالكين' or txtss[2] == 'المالكين' then
DevAbs:del(Revn..'Abs:AbsConstructor:'..msg.chat_id_)
...
DevAbs:del(Revn..'Abs:BasicConstructor:'..msg.chat_id_)
end
end
if BasicConstructor(msg) then
if txts[2] == 'المنشئين' or txtss[2] == 'المنشئين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم حذف المنشئين")  
DevAbs:del(Revn..'Abs:Constructor:'..msg.chat_id_)
end end
if Constructor(msg) then
if txts[2] == 'المدراء' or txtss[2] == 'المدراء' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم حذف المدراء")  
DevAbs:del(Revn..'Abs:Managers:'..msg.chat_id_)
end 
...
DevAbs:srem(Revn..'Abs:Tkeed:'..msg.chat_id_, v)
end 
...
DevAbs:del(Revn..'Abs:Filters:'..msg.chat_id_)
end
if txts[2] == 'قوائم المنع' or txtss[2] == 'قوائم المنع' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌔︙تم حذف قوائم المنع")  
DevAbs:del(Revn..'Abs:Filters:'..msg.chat_id_)
DevAbs:del(Revn.."Abs:FilterAnimation"..msg.chat_id_)
DevAbs:del(Revn.."Abs:FilterPhoto"..msg.chat_id_)
DevAbs:del(Revn.."Abs:FilterSteckr"..msg.chat_id_)
end
...
DevAbs:del(Revn..'Abs:Ban:'..msg.chat_id_) DevAbs:del(Revn..'Abs:Admins:'..msg.chat_id_) DevAbs:del(Revn..'User:Donky:'..msg.chat_id_) DevAbs:del(Revn..'Abs:VipMem:'..msg.chat_id_) DevAbs:del(Revn..'Abs:Filters:'..msg.chat_id_) DevAbs:del(Revn..'Abs:Muted:'..msg.chat_id_)
...
local basicconstructor = DevAbs:smembers(Revn..'Abs:BasicConstructor:'..msg.chat_id_)
local constructor = DevAbs:smembers(Revn..'Abs:Constructor:'..msg.chat_id_)
local Managers = DevAbs:smembers(Revn..'Abs:Managers:'..msg.chat_id_)
local admins = DevAbs:smembers(Revn..'Abs:Admins:'..msg.chat_id_)
local vipmem = DevAbs:smembers(Revn..'Abs:VipMem:'..msg.chat_id_)
local donky = DevAbs:smembers(Revn..'User:Donky:'..msg.chat_id_)
...
spam_c = DevAbs:get(Revn..'Abs:Spam:Text'..msg.chat_id_)
end
--     Source Revn     --
if DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "بالطرد"     
elseif DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "بالتقيد"     
elseif DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "بالكتم"           
elseif DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "بالحذف"
else     
flood = "مفتوح"     
end
--     Source Revn     --
if DevAbs:get(Revn.."Abs:Lock:Bots"..msg.chat_id_) == "del" then
lock_bots = "بالحذف"
elseif DevAbs:get(Revn.."Abs:Lock:Bots"..msg.chat_id_) == "ked" then
lock_bots = "بالتقيد"   
elseif DevAbs:get(Revn.."Abs:Lock:Bots"..msg.chat_id_) == "kick" then
lock_bots = "بالطرد"    
else
lock_bots = "مفتوحه"    
end
--     Source Revn     --
if DevAbs:get(Revn..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه'end
if DevAbs:get(Revn..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفله' else mute_photo = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفله' else mute_video = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفله' else mute_gifs = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفله' else mute_music = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفله' else mute_in = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفله' else mute_voice = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفله' else mute_edit = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفله' else lock_pin = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفله' else lock_sticker = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفله' else lock_tgservice = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفله' else lock_wp = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفله' else lock_htag = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفله' else lock_tag = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفله' else lock_location = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفله' else lock_contact = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفله' else lock_english = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفله' else lock_arabic = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفله' else lock_forward = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Document'..msg.chat_id_) then lock_file = 'مقفله' else lock_file = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفله' else markdown = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفله' else lock_spam = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفل' else lock_Join = 'مفتوح' end
if DevAbs:get(Revn.."Abs:Lock:Welcome"..msg.chat_id_) then send_welcome = 'مقفله' else send_welcome = 'مفتوحه' end
if DevAbs:get(Revn..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end
if DevAbs:get(Revn..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end
if DevAbs:get(Revn..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end
if DevAbs:get(Revn..'Abs:Lock:Farsi'..msg.chat_id_) then lock_farsi = 'مقفله' else lock_farsi = 'مفتوحه' end
local Flood_Num = DevAbs:hget(Revn.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
--     Source Revn     --
...
--     Source Revn     --
if DevAbs:get(Revn..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء حفظ قوانين المجموعه', 1, 'md')
DevAbs:del(Revn..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(Revn..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:rules'..msg.chat_id_,text)
...
DevAbs:set(Revn..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
end
if text and text:match("^القوانين$") then
local rules = DevAbs:get(Revn..'Abs:rules'..msg.chat_id_)
...
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل ميزة انطق'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Antk:Abs'..msg.chat_id_) 
end
if text == "تعطيل انطق" and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل ميزة انطق'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Antk:Abs'..msg.chat_id_,true)  
end
if text and text:match("^انطق (.*)$") and not DevAbs:get(Revn..'Abs:Antk:Abs'..msg.chat_id_) and ChCheck(msg) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Zrf:Abs'..msg.chat_id_) 
end
if text == "تعطيل الزخرفه" and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الزخرفه بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Zrf:Abs'..msg.chat_id_,true)  
end
if DevAbs:get(Revn..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_) then 
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر الزخرفه', 1, 'md')
DevAbs:del(Revn..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_)
return false  
end 
...
if not DevAbs:get(Revn..'Abs:Zrf:Abs'..msg.chat_id_) then
if text == 'زخرفه' and ChCheck(msg) or text == 'الزخرفه' and ChCheck(msg) then  
DevAbs:setex(Revn.."Zrf:Abs"..msg.chat_id_..msg.sender_user_id_,300,true)
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Brg:Abs'..msg.chat_id_) 
end
if text == "تعطيل الابراج" and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الابراج بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Brg:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Revn..'Abs:Brg:Abs'..msg.chat_id_) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Nsba:Abs'..msg.chat_id_) 
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Nsba:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Revn..'Abs:Nsba:Abs'..msg.chat_id_) then
if text == "نسبه الحب" and ChCheck(msg) or text == "نسبة الحب" and ChCheck(msg) then
DevAbs:set(Revn..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙قم بارسل اسمين لحساب نسبة الحب بينهما كمثال ↫ جاك وروز', 1, 'md')
end
end
if text and text ~= "نسبه الحب" and text ~= "نسبة الحب" and DevAbs:get(Revn..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر نسبة الحب ', 1, 'md')
DevAbs:del(Revn..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
...
DevAbs:set(Revn..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙قم بارسل اسمين لحساب نسبة الكره بينهما كمثال ↫ جاك وروز', 1, 'md')
end
end
if text and text ~= "نسبه الكره" and text ~= "نسبة الكره" and DevAbs:get(Revn..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر نسبة الكره ', 1, 'md')
DevAbs:del(Revn..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Revn..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙قم بارسل اسم الشخص لقياس نسبة رجولته كمثال ↫ جاك', 1, 'md')
end
end
if text and text ~= "نسبه الرجوله" and text ~= "نسبة الرجوله" and text ~= "نسبه رجوله" and text ~= "نسبة رجوله" and DevAbs:get(Revn..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر نسبة الرجوله ', 1, 'md')
DevAbs:del(Revn..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Revn..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙قم بارسل اسم الشخص لقياس نسبة انوثته كمثال ↫ روز', 1, 'md')
end
end
if text and text ~= "نسبه الانوثه" and text ~= "نسبة الانوثه" and text ~= "نسبه انوثه" and text ~= "نسبة انوثه" and DevAbs:get(Revn..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر نسبة الانوثه ', 1, 'md')
DevAbs:del(Revn..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Revn..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙قم بارسل اسم الشخص لقياس نسبة غبائه كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الغباء" and text ~= "نسبة الغباء" and DevAbs:get(Revn..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙تم الغاء امر نسبة الغباء ', 1, 'md')
DevAbs:del(Revn..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل حساب العمر'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Age:Abs'..msg.chat_id_) 
end
if text == "تعطيل حساب العمر" and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل حساب العمر'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Age:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Revn..'Abs:Age:Abs'..msg.chat_id_) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Mean:Abs'..msg.chat_id_) 
end
if text == "تعطيل معاني الاسماء" and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل معاني الاسماء'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Mean:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Revn..'Abs:Mean:Abs'..msg.chat_id_) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Audios:Abs'..msg.chat_id_) 
end
if text == "تعطيل غنيلي" and Manager(msg) and ChCheck(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل غنيلي بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Audios:Abs'..msg.chat_id_,true)  
end
if text == "غنيلي" and not DevAbs:get(Revn..'Abs:Audios:Abs'..msg.chat_id_) and ChCheck(msg) then
...
--     Source Revn     --
if Admin(msg) then
if DevAbs:get(Revn..'Abs:LockSettings'..msg.chat_id_) then 
if text == "الروابط" then if DevAbs:get(Revn..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الروابط ↫ "..mute_links.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "المعرف" or text == "المعرفات" then if DevAbs:get(Revn..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفوله' else lock_tag = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙المعرف ↫ "..lock_tag.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "المتحركه" or text == "الملصقات المتحركه" then if DevAbs:get(Revn..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفوله' else mute_gifs = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙المتحركه ↫ "..mute_gifs.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الملصقات" then if DevAbs:get(Revn..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفوله' else lock_sticker = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الملصقات ↫ "..lock_sticker.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الصور" then if DevAbs:get(Revn..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفوله' else mute_photo = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الصور ↫ "..mute_photo.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الفيديو" or text == "الفيديوهات" then if DevAbs:get(Revn..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفوله' else mute_video = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الفيديو ↫ "..mute_video.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الاونلاين" then if DevAbs:get(Revn..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفل' else mute_in = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙الاونلاين ↫ "..mute_in.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الدردشه" then if DevAbs:get(Revn..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الدردشه ↫ "..mute_text.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "التوجيه" or text == "اعاده التوجيه" then if DevAbs:get(Revn..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفل' else lock_forward = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙التوجيه ↫ "..lock_forward.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الاغاني" then if DevAbs:get(Revn..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفوله' else mute_music = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الاغاني ↫ "..mute_music.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الصوت" or text == "الصوتيات" then if DevAbs:get(Revn..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفول' else mute_voice = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙الصوت ↫ "..mute_voice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الجهات" or text == "جهات الاتصال" then if DevAbs:get(Revn..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفوله' else lock_contact = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الجهات ↫ "..lock_contact.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الماركداون" then if DevAbs:get(Revn..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفل' else markdown = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙الماركداون ↫ "..markdown.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الهاشتاك" then if DevAbs:get(Revn..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفل' else lock_htag = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙الهاشتاك ↫ "..lock_htag.."\n"Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "التعديل" then if DevAbs:get(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفل' else mute_edit = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙التعديل ↫ "..mute_edit.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "التثبيت" then if DevAbs:get(Revn..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفل' else lock_pin = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙التثبيت ↫ "..lock_pin.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الاشعارات" then if DevAbs:get(Revn..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفوله' else lock_tgservice = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الاشعارات ↫ "..lock_tgservice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الكلايش" then if DevAbs:get(Revn..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفوله' else lock_spam = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الكلايش ↫ "..lock_spam.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الدخول" then if DevAbs:get(Revn..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفول' else lock_Join = 'مفتوح' end local RevnTEAM = "\n" .."⌔︙الدخول ↫ "..lock_Join.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الشبكات" then if DevAbs:get(Revn..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفوله' else lock_wp = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الشبكات ↫ "..lock_wp.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "المواقع" then if DevAbs:get(Revn..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفوله' else lock_location = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙المواقع ↫ "..lock_location.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "العربيه" then if DevAbs:get(Revn..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفوله' else lock_arabic = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙العربيه ↫ "..lock_arabic.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الانكليزيه" then if DevAbs:get(Revn..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفوله' else lock_english = 'مفتوحه' end local RevnTEAM = "\n" .."⌔︙الانكليزيه ↫ "..lock_english.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الكفر" then if DevAbs:get(Revn..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end local RevnTEAM = "\n" .."⌔︙الكفر ↫ "..lock_kaf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الفشار" then if DevAbs:get(Revn..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end local RevnTEAM = "\n" .."⌔︙الفشار ↫ "..lock_fshar.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
if text == "الطائفيه" then if DevAbs:get(Revn..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end local RevnTEAM = "\n" .."⌔︙الطائفيه ↫ "..lock_taf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, RevnTEAM, 1, 'md') end
end
--     Source Revn     --
if text == 'تفعيل كشف الاعدادات' and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل كشف الاعدادات'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:LockSettings'..msg.chat_id_,true)  
end
if text == 'تعطيل كشف الاعدادات' and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل كشف الاعدادات'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:LockSettings'..msg.chat_id_) 
end
--     Source Revn     --
if text == 'تفعيل اوامر التحشيش' and Manager(msg) and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل اوامر التحشيش'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:Stupid'..msg.chat_id_)
end
if text == 'تعطيل اوامر التحشيش' and Manager(msg) and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل اوامر التحشيش'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:Stupid'..msg.chat_id_,true)
end
--     Source Revn     --
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:Robot'..msg.chat_id_)
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:Robot'..msg.chat_id_,true)
end
--     Source Revn     --
if text == 'تفعيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل ردود المدير'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:GpRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل ردود المدير'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:GpRed'..msg.chat_id_,true)
end
--     Source Revn     --
if text == 'تفعيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل ردود المطور'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:AllRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل ردود المطور'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:AllRed'..msg.chat_id_,true)
end
--     Source Revn     --
if SecondSudo(msg) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn.."Abs:Left:Bot"..Revn)
end
if text == 'تعطيل المغادره' or text == '↫ تعطيل المغادره ⌁' then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل المغادره بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn.."Abs:Left:Bot"..Revn,true) 
end 
if text == 'تفعيل الاذاعه' or text == '↫ تفعيل الاذاعه ⌁' then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل الاذاعه بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn.."Abs:Send:Bot"..Revn)
end
if text == 'تعطيل الاذاعه' or text == '↫ تعطيل الاذاعه ⌁' then 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الاذاعه بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn.."Abs:Send:Bot"..Revn,true) 
end
end
--     Source Revn     --
...
--     Source Revn     --
if msg.content_.photo_ then
if DevAbs:get(Revn..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_) then
if msg.content_.photo_.sizes_[3] then
...
DevAbs:del(Revn..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
send(msg.chat_id_, msg.id_,"⌔︙ليست لدي صلاحية تغير معلومات المجموعه يرجى التحقق من الصلاحيات") 
DevAbs:del(Revn..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(Revn..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
--     Source Revn     --
...
if data.ID == "Ok" then
DevAbs:del(Revn..'Abs:PinnedMsg'..msg.chat_id_)
...
local PinId = DevAbs:get(Revn..'Abs:PinnedMsg'..msg.chat_id_)
if PinId then
...
DevAbs:del(Revn..'Abs:Ban:'..msg.chat_id_)
else
local x = 0
...
local Muted = DevAbs:scard(Revn.."Abs:Muted:"..msg.chat_id_) or "0"
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
tdcli_function({ID="GetChannelFull",channel_id_ = msg.chat_id_:gsub("-100","")},function(arg,data) 
...
local ConstructorList = DevAbs:scard(Revn.."Abs:Constructor:"..ChatId) or 0
local BanedList = DevAbs:scard(Revn.."Abs:Ban:"..ChatId) or 0
local ManagerList = DevAbs:scard(Revn.."Abs:Managers:"..ChatId) or 0
local MutedList = DevAbs:scard(Revn.."Abs:Muted:"..ChatId) or 0
local TkeedList = DevAbs:scard(Revn.."Abs:Abs:Tkeed:"..ChatId) or 0
local AdminsList = DevAbs:scard(Revn.."Abs:Admins:"..ChatId) or 0
local VipList = DevAbs:scard(Revn.."Abs:VipMem:"..ChatId) or 0
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..ChatId))
if LinkGp.ok == true then LinkGroup = LinkGp.result else LinkGroup = 't.me/HATLLA' end
...
ConstructorAbs = "["..Abs.first_name_.."](T.me/"..(Abs.username_ or "HATLLA")..")"
else 
...
ChatLeave(dp.id_, Revn)
DevAbs:srem(Revn.."Abs:Groups", dp.id_)
else
...
DevAbs:set(Revn..'Abs:Num:Add:Bot',Num) 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:FreeBot'..Revn) 
end 
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:FreeBot'..Revn,true) 
end 
end
if ChatType == 'sp' or ChatType == 'gp'  then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn..'Abs:Lock:Games'..msg.chat_id_) 
end
if text == 'تعطيل الالعاب' and Manager(msg) and ChCheck(msg) or text == 'تعطيل اللعبه' and Manager(msg) and ChCheck(msg) then  
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تعطيل الالعاب بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn..'Abs:Lock:Games'..msg.chat_id_,true)  
end
if text == "تفعيل الرابط" or text == "تفعيل جلب الرابط" then 
if Admin(msg) then
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙تم تفعيل جلب رابط المجموعه'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:del(Revn.."Abs:Lock:GpLinks"..msg.chat_id_)
return false  
end
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(Revn.."Abs:Lock:GpLinks"..msg.chat_id_,"ok")
return false  
end
end
--     Source Revn     --
...
ReplyStatus(msg,result.id_,"ReplyBy","⌔︙تم تفعيل المجموعه "..dp.title_)  
DevAbs:sadd(Revn.."Abs:Groups",msg.chat_id_)
if not DevAbs:get(Revn..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_) and not SecondSudo(msg) then 
DevAbs:incrby(Revn..'Abs:Sudos'..msg.sender_user_id_,1)
DevAbs:set(Revn..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_,"abs")
end
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
if not DevAbs:sismember(Revn..'Abs:Groups',msg.chat_id_) then
...
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './Revn.lua', '⌔︙نسخة ملف سورس ريفن',dl_cb, nil)
end end
--     Source Revn     --
...
local Text = "⌔︙Source Revn\n⌔︙File Bot Groups\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
for k,v in pairs(List) do
local GroupsManagers = DevAbs:scard(Revn.."Abs:Managers:"..v) or 0
local GroupsAdmins = DevAbs:scard(Revn.."Abs:Admins:"..v) or 0
local Groupslink = DevAbs:get(Revn.."Abs:Groups:Links" ..v)
...
DevAbs:setex(Revn.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n⌔︙للخروج ارسل ↫ ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Revn.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Revn.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(Revn..'Abs:Users') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
...
DevAbs:setex(Revn.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n⌔︙للخروج ارسل ↫ ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Revn.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Revn.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(Revn..'Abs:Groups') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
...
DevAbs:setex(Revn.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙ارسل الرساله الان لتوجيها \n⌔︙للخروج ارسل ↫ ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Revn.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Revn.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(Revn..'Abs:Groups')   
for k,v in pairs(List) do  
...
DevAbs:setex(Revn.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙ارسل الرساله الان لتوجيها \n⌔︙للخروج ارسل ↫ ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Revn.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Revn.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(Revn..'Abs:Users')   
for k,v in pairs(List) do  
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = msg.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم اذاعة رسالتك بالتوجيه \n⌔︙‏الى ↫ ❨ "..#List.." ❩ مشترك \n ✓", 1, 'md')
...
DevAbs:setex(Revn.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local RevnTEAM = '⌔︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌔︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n⌔︙للخروج ارسل ↫ ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, RevnTEAM, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Revn.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == "الغاء" then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Revn.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
local List = DevAbs:smembers(Revn.."Abs:Groups") 
if msg.content_.text_ then
...
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.text_) 
end
elseif msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
AbsText = "الصوره"
sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(List) do 
AbsText = "المتحركه"
sendDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.video_ then
for k,v in pairs(List) do 
AbsText = "الفيديو"
sendVideo(v, 0, 0, 1, nil, msg.content_.video_.video_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.video_.video_.persistent_id_)
end 
elseif msg.content_.voice_ then
for k,v in pairs(List) do 
AbsText = "البصمه"
sendVoice(v, 0, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.voice_.voice_.persistent_id_)
end 
elseif msg.content_.audio_ then
for k,v in pairs(List) do 
AbsText = "الصوت"
sendAudio(v, 0, 0, 1, nil, msg.content_.audio_.audio_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.audio_.audio_.persistent_id_)
end 
elseif msg.content_.document_ then
for k,v in pairs(List) do 
AbsText = "الملف"
sendDocument(v, 0, 0, 1,nil, msg.content_.document_.document_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.document_.document_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(List) do 
AbsText = "الملصق"
sendSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
DevAbs:set(Revn..'Abs:PinnedMsgs'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
...
local List = DevAbs:smembers(Revn..'Abs:Manager:GpRedod'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙لا توجد ردود متعدده مضافه" ,  1, "md")
return false
end
DevAbs:set(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل كلمة الرد اولا" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local DelGpRedRedod = DevAbs:get(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
if not DevAbs:sismember(Revn..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
...
DevAbs:set(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedods')
DevAbs:set(Revn..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
return false
end end
...
DevAbs:set(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل الكلمه لحذفها" ,  1, "md")
return false
end
if text == 'اضف رد متعدد' and Manager(msg) and ChCheck(msg) then
DevAbs:set(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SetGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل الكلمه الان" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetGpRedod = DevAbs:get(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(Revn..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SaveGpRedod')
DevAbs:set(Revn..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(Revn..'Abs:Manager:GpRedod'..msg.chat_id_,text)
return false
end end
--     Source Revn     --
if text == 'حذف رد' and Manager(msg) and ChCheck(msg) or text == 'مسح رد' and  Manager(msg) and ChCheck(msg) then
local List = DevAbs:smembers(Revn..'Abs:Manager:GpRed'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙لا توجد ردود مضافه" ,  1, "md")
return false
end
DevAbs:set(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'DelGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
if text == 'اضف رد' and Manager(msg) and ChCheck(msg) then
DevAbs:set(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SetGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetGpRed = DevAbs:get(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(Revn..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SaveGpRed')
DevAbs:set(Revn..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(Revn..'Abs:Manager:GpRed'..msg.chat_id_,text)
DevAbs:set(Revn..'DelManagerRep'..msg.chat_id_,text)
return false
end end
--     Source Revn     --
if text == 'حذف رد عام' and SecondSudo(msg) or text == '↫ حذف رد عام ⌁' and SecondSudo(msg) or text == 'مسح رد عام' and SecondSudo(msg) then
local List = DevAbs:smembers(Revn.."Abs:Sudo:AllRed")
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙لا توجد ردود مضافه" ,  1, "md")
return false
end
DevAbs:set(Revn.."Abs:Add:AllRed"..msg.sender_user_id_,'DelAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
if text == 'اضف رد عام' and SecondSudo(msg) or text == '↫ اضف رد عام ⌁' and SecondSudo(msg) then
DevAbs:set(Revn.."Abs:Add:AllRed"..msg.sender_user_id_,'SetAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetAllRed = DevAbs:get(Revn.."Abs:Add:AllRed"..msg.sender_user_id_)
...
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙ارسل لي الرد سواء كان ↫ ⤈\n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n⌔︙يمكنك اضافة الى النص ↫ ⤈\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n `#username` ↬ معرف المستخدم\n `#msgs` ↬ عدد الرسائل\n `#name` ↬ اسم المستخدم\n `#id` ↬ ايدي المستخدم\n `#stast` ↬ رتبة المستخدم\n `#edit` ↬ عدد السحكات\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n⌔︙للخروج ارسل ↫ ( الغاء )\n ✓" ,  1, "md")
DevAbs:set(Revn.."Abs:Add:AllRed"..msg.sender_user_id_,'SaveAllRed')
DevAbs:set(Revn.."Abs:Add:AllText"..msg.sender_user_id_, text)
DevAbs:sadd(Revn.."Abs:Sudo:AllRed",text)
DevAbs:set(Revn.."DelSudoRep",text)
return false 
end end
--     Source Revn     --
if text == 'الردود المتعدده' and Manager(msg) and ChCheck(msg) then
local redod = DevAbs:smembers(Revn..'Abs:Manager:GpRedod'..msg.chat_id_)
MsgRep = '⌔︙قائمة الردود المتعدده ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n'
for k,v in pairs(redod) do
MsgRep = MsgRep..k..'~ (`'..v..'`) • {*العدد ↫ '..#DevAbs:smembers(Revn..'Abs:Text:GpTexts'..v..msg.chat_id_)..'*}\n' 
end
if #redod == 0 then
...
for k,v in pairs(redod) do
DevAbs:del(Revn..'Abs:Text:GpTexts'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Manager:GpRedod'..msg.chat_id_)
end
...
local redod = DevAbs:smembers(Revn..'Abs:Manager:GpRed'..msg.chat_id_)
MsgRep = '⌔︙ردود المدير ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n'
for k,v in pairs(redod) do
if DevAbs:get(Revn.."Abs:Gif:GpRed"..v..msg.chat_id_) then
dp = 'متحركه 🎭'
elseif DevAbs:get(Revn.."Abs:Voice:GpRed"..v..msg.chat_id_) then
dp = 'بصمه 🎙'
elseif DevAbs:get(Revn.."Abs:Stecker:GpRed"..v..msg.chat_id_) then
dp = 'ملصق 🃏'
elseif DevAbs:get(Revn.."Abs:Text:GpRed"..v..msg.chat_id_) then
dp = 'رساله ✉'
elseif DevAbs:get(Revn.."Abs:Photo:GpRed"..v..msg.chat_id_) then
dp = 'صوره 🎇'
elseif DevAbs:get(Revn.."Abs:Video:GpRed"..v..msg.chat_id_) then
dp = 'فيديو 📽'
elseif DevAbs:get(Revn.."Abs:File:GpRed"..v..msg.chat_id_) then
dp = 'ملف 📁'
elseif DevAbs:get(Revn.."Abs:Audio:GpRed"..v..msg.chat_id_) then
dp = 'اغنيه 🎶'
end
...
for k,v in pairs(redod) do
DevAbs:del(Revn..'Abs:Gif:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Voice:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Audio:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Photo:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Stecker:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Video:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:File:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Text:GpRed'..v..msg.chat_id_)
DevAbs:del(Revn..'Abs:Manager:GpRed'..msg.chat_id_)
end
...
local redod = DevAbs:smembers(Revn.."Abs:Sudo:AllRed")
MsgRep = '⌔︙ردود المطور ↫ ⤈ \n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n'
for k,v in pairs(redod) do
if DevAbs:get(Revn.."Abs:Gif:AllRed"..v) then
dp = 'متحركه 🎭'
elseif DevAbs:get(Revn.."Abs:Voice:AllRed"..v) then
dp = 'بصمه 🎙'
elseif DevAbs:get(Revn.."Abs:Stecker:AllRed"..v) then
dp = 'ملصق 🃏'
elseif DevAbs:get(Revn.."Abs:Text:AllRed"..v) then
dp = 'رساله ✉'
elseif DevAbs:get(Revn.."Abs:Photo:AllRed"..v) then
dp = 'صوره 🎇'
elseif DevAbs:get(Revn.."Abs:Video:AllRed"..v) then
dp = 'فيديو 📽'
elseif DevAbs:get(Revn.."Abs:File:AllRed"..v) then
dp = 'ملف 📁'
elseif DevAbs:get(Revn.."Abs:Audio:AllRed"..v) then
dp = 'اغنيه 🎶'
end
MsgRep = MsgRep..k..'~ (`'..v..'`) ↫ {*'..dp..'*}\n' 
end
...
for k,v in pairs(redod) do
DevAbs:del(Revn.."Abs:Add:AllRed"..v)
DevAbs:del(Revn.."Abs:Gif:AllRed"..v)
DevAbs:del(Revn.."Abs:Voice:AllRed"..v)
DevAbs:del(Revn.."Abs:Audio:AllRed"..v)
DevAbs:del(Revn.."Abs:Photo:AllRed"..v)
DevAbs:del(Revn.."Abs:Stecker:AllRed"..v)
DevAbs:del(Revn.."Abs:Video:AllRed"..v)
DevAbs:del(Revn.."Abs:File:AllRed"..v)
DevAbs:del(Revn.."Abs:Text:AllRed"..v)
DevAbs:del(Revn.."Abs:Sudo:AllRed")
end
...
DevAbs:set(Revn..'Abs:NameBot'..msg.sender_user_id_, 'msg')
return false 
end
end
...
DevAbs:set(Revn..'Abs:Help0'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help0'..msg.sender_user_id_)
if RevnTEAM == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help0'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Revn..'Abs:Help01'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help01'..msg.sender_user_id_)
if RevnTEAM == 'msg' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help01'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help1', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Revn..'Abs:Help21'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help21'..msg.sender_user_id_)
if RevnTEAM == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help21'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help2', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Revn..'Abs:Help31'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help31'..msg.sender_user_id_)
if RevnTEAM == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help31'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help3', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Revn..'Abs:Help41'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help41'..msg.sender_user_id_)
if RevnTEAM == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help41'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help4', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
DevAbs:set(Revn..'Abs:Help51'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help51'..msg.sender_user_id_)
if RevnTEAM == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help51'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help5', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Revn..'Abs:Help61'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local RevnTEAM =  DevAbs:get(Revn..'Abs:Help61'..msg.sender_user_id_)
if RevnTEAM == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Revn..'Abs:Help61'..msg.sender_user_id_)
DevAbs:set(Revn..'Abs:Help6', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙جاري تحديث سورس ريفن', 1, 'md') 
os.execute('rm -rf Revn.lua') 
os.execute('wget https://raw.githubusercontent.com/RevnTame/Revn/master/Revn.lua') 
dofile('Revn.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/RevnTame/RevnFiles/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
vardump(res.plugins_)
if Get_info then
local TextS = "\n⌔︙قائمة ملفات متجر سورس ريفن\n⌔︙الملفات المتوفره حاليا ↫ ⤈\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\n"
...
local GetJson, Res = https.request("https://raw.githubusercontent.com/RevnTame/RevnFiles/master/RevnFiles/"..FileName)
if Res == 200 then
...
DevAbs:del(Revn..'Abs:Text:BotWelcome')
DevAbs:del(Revn..'Abs:Photo:BotWelcome')
return false
end 
...
DevAbs:del(Revn..'Abs:Lock:BotWelcome')
return false
end 
...
DevAbs:set(Revn..'Abs:Lock:BotWelcome',true)
return false
end 
...
DevAbs:del(Revn..'Abs:Text:BotWelcome')
DevAbs:del(Revn..'Abs:Photo:BotWelcome')
DevAbs:set(Revn.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Text") 
return false
end 
if text and DevAbs:get(Revn.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Text' then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء الامر", 1, "md") 
DevAbs:del(Revn.."Abs:Set:BotWelcome"..msg.sender_user_id_)   
return false
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم حفظ النص ارسل لي صورة الترحيب\n⌔︙ارسل ↫ الغاء لحفظ النص فقط", 1, 'md')   
DevAbs:set(Revn.."Abs:Text:BotWelcome",text) 
DevAbs:set(Revn.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Photo") 
return false 
end 
if DevAbs:get(Revn.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Photo' then 
if text and text:match("^الغاء$") then 
...
DevAbs:set(Revn.."Abs:Photo:BotWelcome",msg.content_.photo_.sizes_[1].photo_.persistent_id_)
...
DevAbs:setex(Revn.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_, 300, true)
end end
...
DevAbs:del(Revn.."DevText")
end end
--     Source Revn     --
if DevAbs:get(Revn.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌔︙تم الغاء الامر", 1, "md") 
DevAbs:del(Revn.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del(Revn.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local texxt = string.match(text, "(.*)") 
DevAbs:set(Revn..'Abs:ChText',texxt)
...
DevAbs:setex(Revn.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌔︙Channel ↬ [@HATLLA]', 1, 'md')    
end 
--     Source Revn     --
...
DevAbs:incr(Revn..'Abs:EditMsg'..result.chat_id_..result.sender_user_id_)
local text = result.content_.text_ or result.content_.caption_
local Text = result.content_.text_
if DevAbs:get(Revn..'Abs:Lock:EditMsgs'..msg.chat_id_) and not Text and not BasicConstructor(result) then
...
local GetMsgPin = DevAbs:get(Revn..'Abs:PinnedMsgs'..msg.chat_id_)
if GetMsgPin ~= nil then
if text == GetMsgPin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) if dp.ID == 'Ok' then;DevAbs:del(Revn..'Abs:PinnedMsgs'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if GetMsgPin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) DevAbs:del(Revn..'Abs:PinnedMsgs'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_=v,user_id_=Revn,status_={ID = "ChatMemberStatusLeft"},},function(e,g) end, nil) 
DevAbs:srem(Revn..'Abs:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
DevAbs:srem(Revn..'Abs:Groups',v)  
end
